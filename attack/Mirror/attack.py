...
def blackbox_attack(genforce_name, target_name, eval_name, target_labels, work_dir, result_dir=None, batch_size=10, device='cpu'):
    ...
    ckpt_dir = os.path.join(work_dir, 'model')
    ...
    if len(os.listdir(presample_dir)) == 0:
        presample(presample_dir, genforce_name, ckpt_dir, sample_num=8000, batch_size=40)
        
    mirror_blackbox_attack(True, 1000, target_name, genforce_name, ckpt_dir, presample_dir, target_labels, cache_dir, ckpt_dir, batch_size=batch_size, use_cache=False, device=device)
