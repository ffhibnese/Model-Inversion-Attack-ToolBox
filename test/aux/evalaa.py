...
class TmpDataset(Dataset):
    ...
    def __init__(self, labels, files) -> None:
        super().__init__()
        self.labels = labels
        self.files = files
    ...
    def __getitem__(self, index) -> Any:
        return torch.load(self.files[index], map_location='cpu'), torch.LongTensor(self.labels[index])
        
@torch.no_grad()
def main():
    root_path = '/data/yuhongyao/Model_Inversion_Attack_ToolBox/results/gmi_high_metfaces/facescrub_resnet18_metfaces/all_imgs'
    all_tensors = []
    # all_tensor_paths = []
    all_labels = []
    import torchvision
    ...
    for label in tqdm(os.listdir(root_path)):
        ...
        for filename in os.listdir(label_dir):
            if filename.endswith('.pt'):
                ...
                # all_tensor_paths.append(filepath)
                # print(tensor.shape)
    ...
    # ds = TmpDataset(all_labels, all_tensor_paths)
    # loader = DataLoader(ds, 50, shuffle=False)
    all_tensors = torch.stack(all_tensors, dim=0)
    all_labels = torch.LongTensor(all_labels)
    # all_tensors, all_labels = [], []
    # # all_tensors, all_labels = next(iter(loader))
    # for tensors, labels in tqdm(loader):
...
def faa(all_tensors, all_labels):
    eval_model = Classifier(num_classes=530, architecture='inception_v3')
    eval_model.load_state_dict(torch.load('../intermediate-MIA/intermediate-MIA/pretrained/inception_v3_facescrub.pt'))
    eva = evaluator(eval_model, 'facescrub', 20, gpu_devices=[0])
    print('start evaluate')
