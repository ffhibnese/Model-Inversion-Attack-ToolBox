...
class StyleGan2adaMappingWrapper(nn.Module):
    ...
    def __init__(self, mapping, single_w, truncation_psi=0.5, truncation_cutoff=8, *args, **kwargs) -> None:
    ...
    def forward(self, z):
        w = self.mapping(z, c=None, truncation_psi = self.truncation_psi, truncation_cutoff=self.truncation_cutoff)
        if self.single_w:
            w = w[:, [0]]
        return w
...
class StyleGAN2adaSynthesisWrapper(BaseIntermediateImageGenerator):
    ...
    def _forward_impl(self, ws: Tensor, intermediate_inputs: Optional[Tensor] = None, labels: torch.LongTensor | None = None, start_block: int = None, end_block: int = None, noise_mode='const', force_fp32=True, **kwargs):
...
def get_stylegan2data_generator(stylegan2ada_path: str, checkpoint_path: str, single_w=True, truncation_psi=0.5, truncation_cutoff=8):
    
    sys.path.append(stylegan2ada_path)
    
    with open(checkpoint_path, 'rb') as f:
        G = pickle.load(f)['G_ema']
    mapping = StyleGan2adaMappingWrapper(G.mapping, single_w, truncation_psi, truncation_cutoff)
