...
class LoktSurrogateTrainArgs(BaseTrainArgs):
    ...
    augment: Callable = field(
        default_factory=lambda: kornia.augmentation.container.ImageSequential(
...
class LoktSurrogateTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        dataset_dir = os.path.join(
            self.folder_manager.config.cache_dir,
            'surrogate_dataset',
            args.target_dataset_name,
        )

        self.dataset_path = os.path.join(
            dataset_dir, f'{args.target_name}_{args.target_defense_type}.pt'
        )
        ...
        if not os.path.exists(self.dataset_path):
            ...
            self.folder_manager.load_state_dict(
                self.G,
                ['lokt', f'lokt_{args.dataset_name}_{args.target_name}_G.pt'],
                args.device,
                self.args.defense_type,
            )
            ...
            T = get_model(
                ...
                args.target_dataset_name,
                args.device,
                defense_type=args.target_defense_type,
            )
            self.folder_manager.load_target_model_state_dict(
                T,
                self.args.target_dataset_name,
                args.target_name,
                args.device,
                args.defense_type,
            )

            pseudo_ys = torch.arange(
                0, self.num_classes, dtype=torch.long
            ).repeat_interleave(args.samples_per_class)
            imgs, pred_labels = [], []
            for start_idx in tqdm(range(0, len(pseudo_ys), args.sample_batch_size)):
                ...
                img, pred_label = self._create_batch_dataset(
                    T, pseudo_ys[start_idx:end_idx]
                )
                imgs.append(img)
                pred_labels.append(pred_label)
            imgs = torch.cat(imgs, dim=0)
            pred_labels = torch.cat(pred_labels, dim=0)
            torch.save({'imgs': imgs, 'target_y': pred_labels}, self.dataset_path)
    ...
    def save_state_dict(self):
        ...
        self.folder_manager.save_state_dict(
            model,
            [
                'lokt',
                f'{self.args.model_name}_{self.args.target_dataset_name}_{self.args.target_name}_{self.args.target_defense_type}.pt',
            ],
            self.args.defense_type,
        )
