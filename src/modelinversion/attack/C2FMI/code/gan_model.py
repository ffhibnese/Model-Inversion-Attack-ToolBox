...
class ConstantInput(nn.Module):
    ...
    def forward(self, input):
        batch = input.shape[0]
        out = self.input.repeat(batch, 1, 1, 1)  # 复制batch个512*4*4的input
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, noise=None):  # 调制卷积，加噪，激活
...
class Generator(nn.Module):
    def __init__(
        ...
        # 加了8个 EqualLinear=nn.Linear + activation, 自定义学习率
        ...
        self.style = nn.Sequential(*layers)  # f:z->w
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(out_channel, style_dim))  # ToRGB 层

            in_channel = out_channel

        self.n_latent = self.log_size * 2 - 2  # 风格隐向量数量

    # 随机生成各层噪声 加噪层数*1(batch)*1(channel)*h*w
    def make_noise(self):
    ...
    def forward(
        ...
        if noise is None:
            if randomize_noise:
                ...
                ]  # 不随机噪声，返回noise为self.noises的属性值noise_{i}

        # 是否使用 truncation （==1为使用）
        if truncation < 1:
            style_t = []
        ...
        # 如果不使用混合向量，混合向量:styles=[batch*dim,batch*dim]，非混合向量：styles=[batch*dim]
        if len(styles) < 2:
...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        out = self.convs(input)  # batch*512*4*4
