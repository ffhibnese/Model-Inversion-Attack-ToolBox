...
class KEDMIAttacker(BaseAttacker):
    ...
    def prepare_attack(self):
        ...
        self.folder_manager.load_state_dict(
            ...
            [
                'KEDMI',
                f'{config.gan_dataset_name}_{config.gan_target_name.upper()}_KEDMI_D.tar',
            ],
            device=config.device,
        )
    ...
    def get_loss(self, fake, iden):
        ...
        Prior_Loss = torch.mean(F.softplus(self.log_sum_exp(D_label))) - torch.mean(
            self.log_sum_exp(D_label)
        )
        ...
        return {'total': Total_Loss, 'prior': Prior_Loss, 'iden': Iden_Loss}
    ...
    def attack_step(self, iden) -> dict:
        ...
        return {'acc': acc, 'acc5': acc_5, 'acc_var': acc_var, 'acc5_var': acc_var5}
