...
class RLBMIAttacker(BaseAttacker):
    ...
    def __init__(self, config: RLBMIAttackConfig) -> None:
        self._tag = (
            f'{config.dataset_name}_{config.target_name}_{config.gan_dataset_name}'
        )
    ...
    def attack_step(self, iden) -> dict:
        ...
        for i in iden:
            agent = Agent(
                state_size=config.z_dim,
                action_size=config.z_dim,
                device=config.device,
                hidden_size=256,
    ...
    def _inversion(self, agent: Agent, label: int):
        ...
        for iter_time in tqdm(range(config.iter_times)):
            ...
            for t in range(config.max_step):
                ...
                z = alpha * z + (1 - alpha) * action.clone().detach().reshape(
                    (1, len(action))
                ).to(device)
                next_state = deepcopy(z.cpu().numpy())
                state_image = self.G(z).detach()
                action_image = self.G(
                    action.clone().detach().reshape((1, len(action))).to(device)
                ).detach()
                ...
                score3 = math.log(
                    max(
                        ...
                        float(
                            torch.index_select(
                                F.softmax(state_output, dim=-1).data, 1, y
                            )
                        )
                        - float(
                            torch.max(
