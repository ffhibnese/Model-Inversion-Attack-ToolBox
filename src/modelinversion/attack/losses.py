...
from ..models import BaseImageClassifier, GmiDiscriminator64, GmiDiscriminator256, KedmiDiscriminator64, KedmiDiscriminator256
...
class ImageAugmentClassificationLoss(BaseImageLoss):
    ...
    def __init__(self, classifier: BaseImageClassifier, loss_fn: str | Callable[[Tensor, LongTensor], Tensor] = 'cross_entropy', create_aug_images_fn: Optional[Callable[[Tensor], Iterable[Tensor]]]=None, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
...
class GmiDiscriminatorLoss(BaseImageLoss):
    ...
    def __init__(self, discriminator: GmiDiscriminator64 | GmiDiscriminator256, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
...
class KedmiDiscriminatorLoss(BaseImageLoss):
    ...
    def __init__(self, discriminator: KedmiDiscriminator64 | KedmiDiscriminator256, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.discriminator = discriminator
...
class ComposeImageLoss(BaseImageLoss):
    ...
    def __init__(self, losses: list[BaseImageLoss], weights: Optional[list[float]] = None, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        if weights is None:
        ...
        if len(weights) != len(losses):
            raise RuntimeError(f'Expect the equal length of losses and weights, but found the fronter {len(losses)} and the latter {len(weights)}.')
