...
def inversion(G, D, T, E, iden, img_size, folder_manager: FolderManager, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, num_seeds=5, device='cpu'):

    mpv = np.array([0.5189, 0.4346, 0.3886])
    mpv = torch.tensor(mpv.astype(np.float32).reshape(1, 3, 1, 1)).cuda()

    # 获取label信息、交叉熵损失以及batchsize
    iden = iden.view(-1).long().to(device)
    criterion = nn.CrossEntropyLoss().to(device)
    bs = iden.shape[0]
    
    # 用于数据增强操作
    easy_cutout = partial(cutout, mpv=mpv, img_size=img_size, bs=bs)
