from .config import SecretGenConfig
import os
...
def attack(config: SecretGenConfig):
    ...
    # evaluate on the first 300 identities only
        for idx in range((len(config.target_labels) - 1) // config.batch_size + 1):
            ...
            acc, acc5, var, var5 = inversion(G, D, T, E, iden, img_size=config.img_size, folder_manager=folder_manager, 
                                                lr=2e-2, 
                                                momentum=0.9, 
                                                lamda=100,
                                                iter_times=1500, 
