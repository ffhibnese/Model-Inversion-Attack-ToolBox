...
class PPAAttacker(BaseSingleLabelAttacker):
    ...
    def synthesize(self, w, num_ws):
        if w.shape[1] == 1:
            w_expanded = torch.repeat_interleave(w, repeats=num_ws, dim=1)
            imgs = self.synthesis_module(
                w_expanded, noise_mode='const', force_fp32=True
            )
        else:
    ...
    def _get_init_w_path(self, target):
        ...
        w_dir = os.path.join(
            self.folder_manager.config.cache_dir, w_tag, f'{config.num_candidate}'
        )
    ...
    def _create_initial_ws(self, targets, mapping_module):
        ...
        gen_targets = [
            ...
            if not os.path.exists(self._get_init_w_path(target))
        ]
        ...
        for start_idx in tqdm(
            range(0, config.init_select_num, config.init_select_batch_size)
        ):
            end_idx = min(
                start_idx + config.init_select_batch_size, config.init_select_num
            )
            z_batch = z[start_idx:end_idx].to(config.device)
            w_batch = mapping_module(
                z_batch,
                c,
                truncation_psi=config.truncation_psi,
                truncation_cutoff=config.truncation_cutoff,
            )
    ...
    def _optimize(self, w: torch.Tensor, target: int):
        ...
        for epoch in tqdm(range(config.num_epochs)):
            ...
            labels = (
                torch.ones((len(outputs),), dtype=torch.long, device=config.device)
                * target
            )
        ...
        with torch.no_grad():
            ...
            for w_batch_optimized in w_optimized:
                ...
                for i in tqdm(range(config.final_select_iters)):
                    ...
                    aug_imgs_batch = (
                        imgs_batch
                        if config.final_select_transform is None
                        else config.final_select_transform(imgs_batch)
                    )
                    scores_batch += (
                        torch.softmax(self.T(aug_imgs_batch).result, dim=-1)[
                            ..., target
                        ]
                        .detach()
                        .cpu()
            ...
            top_scores, top_indice = torch.topk(
                scores, k=config.samples_per_targets, dim=-1
            )
            ...
            safe_save(
                top_ws,
                os.path.join(self.folder_manager.config.result_dir, 'final_w'),
                f'{target}.pt',
            )
