...
class BrepAttacker(BaseAttacker):
    ...
    def prepare_attack(self):
        ...
        self.folder_manager.load_state_dict(
            ...
            [
                'KEDMI',
                f'{config.gan_dataset_name}_{config.gan_target_name.upper()}_KEDMI_G.tar',
            ],
    ...
    def init_z(self):
        ...
        with torch.no_grad():
            ...
            for current_iter in tqdm(range(config.init_z_max_iter)):
                z = (
                    torch.randn(batch_size, config.z_dim)
                    .float()
                    .clamp(min=config.point_clamp_min, max=config.point_clamp_max)
                    .to(config.device)
                )
                first_img = self.G(z)
                ...
                for i in range(batch_size):
                    ...
                    if (
                        current_label in initial_points
                        or current_label not in target_labels
                    ):
                        continue
                ...
                # current_iter += 1
                if (
                    len(initial_points) == num_idens
                ):  # or current_iter > config.init_z_max_iter:
                    break
            ...
            print(
                f'labels {str(unmap_labels)} can not be generate in iter {config.init_z_max_iter}'
            )
    ...
    def _gen_points_on_sphere(self, current_point, points_count, sphere_radius, device):
        ...
        perturbation_direction = (
            sphere_radius
            / torch.sqrt(torch.sum(perturbation_direction**2, axis=dims, keepdims=True))
        ) * perturbation_direction
    ...
    def attack_single(self, label, initial_z):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(config.max_iters_at_radius_before_terminate)):
                ...
                new_points, perturbation_directions = self._gen_points_on_sphere(
                    current_z[0],
                    config.sphere_points_count,
                ...
                new_points_classification = self.is_target_class(
                    self.G(new_points), label, self.T
                )
                ...
                grad_direction = (
                    torch.mean(
                        new_points_classification.unsqueeze(1)
                ...
                z_new = z_new.clamp(
                    min=config.point_clamp_min, max=config.point_clamp_max
                )
