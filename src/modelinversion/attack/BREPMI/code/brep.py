...
def is_target_class(idens, target, model, score=False, criterion=None):
    if score:
        ...
        val_iden, score = decision(
            idens, model, score, target_class_tensor, criterion=criterion
        )
    else:
...
def brep_single_attack(
    args: BrepArgs, G, T, E, z, label, creterion, folder_manager: FolderManager, device
) -> bool:
    current_iter = 0
    ...
    with torch.no_grad():
        while (
            current_iter - last_iter_when_radius_changed
            < args.max_iters_at_radius_before_terminate
        ):
            ...
            new_points, perturbation_directions = gen_points_on_sphere(
                current_z[0],
                args.sphere_points_count,
            ...
            grad_direction = (
                torch.mean(
                    new_points_classification.unsqueeze(1) * perturbation_directions,
                    axis=0,
                )
                / current_sphere_radius
            )
...
def brep_attack(
    args: BrepArgs, G, T, E, target_labels, folder_manager: FolderManager, device
):
    init_z_ = init_z(
        args.batch_dim_for_initial_points,
        G,
        T,
        args.point_clamp_min,
        args.point_clamp_max,
        args.z_dim,
        target_labels,
        device=device,
    )
    ...
    for label in tqdm(init_z_.keys()):
        ...
        correct_on_eval += brep_single_attack(
            args, G, T, E, z, label, criterion, folder_manager, device=device
        )
