...
def attack(config: BrepAttackConfig):
    ...
    save_dir = os.path.join(
        config.result_dir,
        f'{config.dataset_name}_{config.target_name}_{config.gan_target_name}',
    )
    ...
    folder_manager.load_target_model_state_dict(
        E, config.dataset_name, config.eval_name, device=config.device
    )

    G = Generator(args.z_dim).to(config.device)
    folder_manager.load_state_dict(
        G,
        [
            'KEDMI',
    ...
    acc = brep_attack(
        args, G, T, E, config.target_labels, folder_manager, device=config.device
    )
