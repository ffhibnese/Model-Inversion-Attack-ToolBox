...
class PlgmiGANTrainer(BaseGANTrainer):
    ...
    def __init__(self, args: PlgmiGANTrainArgs, folder_manager, **kwargs) -> None:
        ...
        self.src_dataset_dir = os.path.join(
            folder_manager.config.dataset_dir, args.dataset_name, 'split', 'public'
        )
        self.dst_dataset_dir = os.path.join(
            folder_manager.config.cache_dir, args.dataset_name, args.target_name
        )
    ...
    def prepare_training(self):
        ...
        self.D = SNResNetProjectionDiscriminator(num_classes=self.num_classes).to(
            args.device
        )
        self.T = get_model(
            args.target_name,
        ...
        if not self._check_select_topn():
            print(
                f'start top n selection from {self.src_dataset_dir} to {self.dst_dataset_dir}'
            )
            src_img_paths = walk_imgs(self.src_dataset_dir)
            trans = tv_trans.Compose([Image.open, tv_trans.ToTensor()])
    ...
    def train_gen_step(self, batch):
        ...
        # }
        return OrderedDict(
            dis_loss=dis_loss.item(), inv_loss=inv_loss.item(), total_loss=loss.item()
        )
    ...
    def train_dis_step(self, batch):
        ...
        dis_fake = torch.mean(torch.relu(1.0 + dis_fake))
        ...
        dis_real = torch.mean(torch.relu(1.0 - dis_real))
        ...
        # }
        return OrderedDict(
            fake_loss=dis_fake.item(), real_loss=dis_real.item(), total_loss=loss.item()
        )
