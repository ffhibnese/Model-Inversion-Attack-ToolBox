...
def get_args():
    parser = argparse.ArgumentParser(
        description='Stage-1: Train the Pseudo Label-Guided Conditional GAN'
    )
    ...
    parser.add_argument(
        '--private_data_root',
        type=str,
        default='datasets/celeba_private_domain',
    ...
    # Resume training
    parser.add_argument(
        '--args_path', default=None, help='Checkpoint args json path. default: None'
    )
    parser.add_argument(
        '--gen_ckpt_path',
        '-gcp',
    ...
    # Model Inversion
    parser.add_argument(
        '--alpha', type=float, default=0.2, help='weight of inv loss. default: 0.2'
    )
    parser.add_argument(
        '--inv_loss_type', type=str, default='margin', help='ce | margin | poincare'
    )
...
def main():
    ...
    target_model.load_state_dict(
        torch.load(target_model_path)['state_dict'], strict=False
    )
    target_model.eval()
    ...
    evaluate_model.load_state_dict(
        torch.load(evaluate_model_path)['state_dict'], strict=False
    )
    evaluate_model.eval()
    ...
    inception_model = (
        inception.InceptionV3().to(device) if args.calc_FID else None
    )  # Calc FID need
    # load optimizer
    ...
    for n_iter in range(1, args.max_iteration + 1):
            ...
            if i == 0:
                ...
                if args.inv_loss_type == 'ce':
                    inv_loss = L.cross_entropy_loss(
                        target_model(fake_aug)[-1], pseudo_y
                    )
        ...
        if n_iter % args.log_interval == 0:
            ...
            utils.save_images(
                n_iter,
                n_iter // args.checkpoint_interval,
                args.results_root,
