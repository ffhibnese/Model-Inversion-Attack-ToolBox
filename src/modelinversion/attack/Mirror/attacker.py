...
class MirrorBlackboxAttacker(BaseAttacker):
    ...
    def prepare_attack(self):
        ...
        self.genforce_model, _ = get_genforce(
            self.config.genforce_name,
            self.config.device,
            self.folder_manager.config.ckpt_dir,
            use_discri=False,
        )
        ...
        if (
            not os.path.exists(check_presample_dir)
            or len(os.listdir(check_presample_dir)) == 0
        ):
            print('presample')
            presample(
                presample_dir,
                self.genforce_model,
                batch_size=self.config.presample_batch_size,
                device=self.config.device,
            )

    def attack_step(self, iden) -> dict:
        # return super().attack_step(iden)
        return mirror_blackbox_attack(
            self.config, iden, self.genforce_model, self.T, self.E, self.folder_manager
        )
...
class MirrorWhiteboxAttacker(BaseAttacker):
    ...
    def attack_step(self, iden) -> dict:
        ...
        return mirror_white_box_attack(
            self.config,
            self.genforce_model,
            self.T,
            self.E,
            self.folder_manager,
            iden,
            to_target_transforms=self.to_target_transforms,
        )
