...
def find_closest_latent(
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(len(all_ws_gen_files))):
            ...
            fake = normalize(
                resize_img(fake * 255.0, target_model.resolution), arch_name
            )
            ...
            for i in range(0, len(fake), bs):
                ...
                output = F.softmax(target_model(inputs.to(device)).result, dim=-1)[
                    :, targets_list
                ]
    ...
    target_ranked_confidence_dict = {
        t: torch.cat(v, dim=0) for t, v in target_ranked_confidence_dict.items()
    }
    ...
    for t, v in target_ranked_confidence_dict.items():
        ...
        for top_id in topk_idx:
            res_w[t].append(
                torch.load(all_ws_gen_files[top_id // pt_num])[top_id % pt_num]
            )
