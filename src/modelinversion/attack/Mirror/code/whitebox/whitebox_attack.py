...
def mirror_white_box_attack(
    ...
    if args.genforce_name.startswith('stylegan'):
        ...
        invert_lrelu = nn.LeakyReLU(5.0)
        lrelu = nn.LeakyReLU(0.2)

        use_p_space_bound = args.p_std_ce > 0.0 and use_w_space and not use_z_plus_space
        ...
        if use_p_space_bound:
            ...
            for ws_file in all_ws_gen_files:
                ...
                all_ws = (
                    torch.load(ws_file, map_location=args.device)
                    .detach()
                    .to(args.device)
                )
    ...
    if use_pre_generated_latents:
        ...
        w_dict = find_closest_latent(
            target_net,
            args.device,
            target_list,
            nrow,
            args.target_name,
            folder_manager.config.presample_dir,
            to_target_transforms=to_target_transforms,
        )[0]
        ...
        if use_w_space:
            if use_z_plus_space:
                ...
                if use_w_mean:
                    ...
                    with torch.no_grad():
                        latent_z_inputs = torch.randn(
                            n_mean_latent, latent_dim, device=args.device
                        )
                    ...
                    with torch.no_grad():
                        latent_z_inputs = torch.randn(
                            batch_size, latent_dim, device=args.device
                        )
                        latent_w = generator.G.mapping(latent_z_inputs)['w']
                        print(f'latent_z_inputs.shape: {latent_z_inputs.shape}')
                        print(f'latent_w.shape: {latent_w.shape}')
                ...
                if not repeat_w:
                    inputs = inputs.unsqueeze(1).repeat(
                        1, w_num_layers, 1
                    )  # shape: batch_size x num_layers x 512
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        input_images = normalize(fake * 255.0, args.target_name)
        ...
        if epoch % args.save_every == 0:
            ...
            with torch.no_grad():
                ...
                fake = crop_img(fake, args.target_name)
                target_fake = normalize(fake * 255.0, args.target_name)
                if to_target_transforms is not None:
                    target_fake = to_target_transforms(target_fake)
                target_acc = verify_acc(
                    target_fake, targets, target_net, args.target_name
                )
                print(f'target_acc: {target_acc:.6f}')

                eval_fake = normalize(fake * 255.0, args.target_name)
    ...
    with torch.no_grad():
        ...
        fake = normalize(fake * 255.0, args.target_name)
    ...
    for i in range(fake.shape[0]):
        ...
        image = denormalize(
            fake[i], args.target_name
        )  # .data.cpu().numpy().transpose((1, 2, 0))
