    ...
    def forward(self, image, lod=None, **_unused_kwargs):
        ...
        for res_log2 in range(self.final_res_log2, self.init_res_log2 - 1, -1):
            ...
            if current_lod <= lod < current_lod + 1:
                ...
                x = self.__getattr__(f'input{block_idx}')(image) * alpha + x * (
                    1 - alpha
                )
...
class DownsamplingLayer(nn.Module):
    ...
    def forward(self, x):
        if self.scale_factor <= 1:
            return x
        return F.avg_pool2d(
            x, kernel_size=self.scale_factor, stride=self.scale_factor, padding=0
        )
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.conv2d(
            x, weight=weight, bias=self.bias, stride=self.stride, padding=self.padding
        )
        x = self.activate(x)
        x = self.downsample(x)
        return x
