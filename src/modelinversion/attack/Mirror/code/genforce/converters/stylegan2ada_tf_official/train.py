...
def setup_training_options(
    ...
    with tf.Graph().as_default(), tflib.create_session().as_default():  # pylint: disable=not-context-manager
        ...
        args.train_dataset_args.max_label_size = (
            dataset_obj.label_size
    ...
    args.G_args.num_fp16_res = args.D_args.num_fp16_res = (
        4  # enable mixed-precision training
    )
    args.G_args.conv_clamp = args.D_args.conv_clamp = (
    ...
    if cmethod == 'nocmethod':
        ...
        args.G_args.num_fp16_res = args.D_args.num_fp16_res = (
            0  # disable mixed-precision training
        )
        ...
        args.D_args.score_max = (
            5  # prepare D to output 5 scalars per image instead of just 1
        )
...
def run_training(outdir, seed, dry_run, **hyperparam_options):
    ...
    with open(
        os.path.join(training_options.run_dir, 'training_options.json'), 'wt'
    ) as f:
...
def main():
    ...
    group.add_argument(
        '--outdir',
        help='Where to save the results (required)',
        required=True,
        metavar='DIR',
    )
    group.add_argument(
        '--gpus', help='Number of GPUs to use (default: 1 gpu)', type=int, metavar='INT'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--data', help='Training dataset path (required)', metavar='PATH', required=True
    )
    group.add_argument(
        '--res',
        help='Dataset resolution (default: highest available)',
        type=int,
        metavar='INT',
    )
    group.add_argument(
        '--mirror',
        help='Augment dataset with x-flips (default: false)',
        type=_str_to_bool,
        metavar='BOOL',
    )
    ...
    group.add_argument(
        '--metricdata',
        help='Dataset to evaluate metrics against (optional)',
        metavar='PATH',
    )
    ...
    group.add_argument(
        ...
        choices=['noaug', 'ada', 'fixed', 'adarv'],
    )
    group.add_argument(
        '--p',
        help='Specify augmentation probability for --aug=fixed',
        type=float,
        metavar='FLOAT',
    )
    group.add_argument(
        '--target', help='Override ADA target for --aug=ada and --aug=adarv', type=float
    )
    group.add_argument(
        '--augpipe',
    ...
    group.add_argument(
        '--cmethod',
        help='Comparison method (default: nocmethod)',
        choices=[
            'nocmethod',
            'bcr',
            'zcr',
    ...
    group.add_argument(
        '--dcap',
        help='Multiplier for discriminator capacity',
        type=float,
        metavar='FLOAT',
    )
    ...
    group.add_argument(
        '--resume', help='Resume from network pickle (default: noresume)'
    )
    group.add_argument(
        '--freezed',
        help='Freeze-D (default: 0 discriminator layers)',
        type=int,
        metavar='INT',
    )
