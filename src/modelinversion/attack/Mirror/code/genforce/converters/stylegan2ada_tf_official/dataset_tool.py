    ...
    def add_image(self, img):
        ...
        if self.shape is None:
            ...
            tfr_opt = tf.io.TFRecordOptions(
                tf.compat.v1.io.TFRecordCompressionType.NONE
            )
...
class HDF5Exporter:
    ...
    def add_images(self, img):
        assert (
            img.ndim == 4
            and img.shape[1] == self.channels
            and img.shape[2] == img.shape[3]
        )
        assert img.shape[2] >= self.resolution and img.shape[2] == 2 ** int(
            np.floor(np.log2(img.shape[2]))
        )
        if (
            self.print_progress
            and (self.cur_images - 1) % self.progress_interval
            >= self.progress_interval - img.shape[0]
        ):
            print(
                '%d / %d\r' % (self.cur_images, self.expected_images),
                end='',
        ...
        for lod in range(len(self.h5_lods)):
            ...
            while ofs < quant.shape[0]:
                num = min(
                    quant.shape[0] - ofs,
                    self.buffers[lod].shape[0] - self.buffer_sizes[lod],
                )
                self.buffers[lod][
                    self.buffer_sizes[lod] : self.buffer_sizes[lod] + num
                ] = quant[ofs : ofs + num]
...
def info(tfrecord_dir):
    ...
    dset = dataset.TFRecordDataset(
        tfrecord_dir, max_label_size='full', repeat=False, shuffle=False
    )
    ...
    print(
        ...
        % (
            'Label range:',
            (
                '%g -- %g' % (label_min, label_max)
                if num_images and dset.label_size
                else 'n/a'
            ),
        )
    )
...
def extract(tfrecord_dir, output_dir):
    ...
    tflib.init_tf()
    dset = dataset.TFRecordDataset(
        tfrecord_dir, max_label_size=0, repeat=False, shuffle=False
    )
    tflib.init_uninitialized_vars()
...
def compare(tfrecord_dir_a, tfrecord_dir_b, ignore_labels):
    ...
    dset_a = dataset.TFRecordDataset(
        tfrecord_dir_a, max_label_size=max_label_size, repeat=False, shuffle=False
    )
    print('Loading dataset "%s"' % tfrecord_dir_b)
    dset_b = dataset.TFRecordDataset(
        tfrecord_dir_b, max_label_size=max_label_size, repeat=False, shuffle=False
    )
    tflib.init_uninitialized_vars()
...
def pack(
    ...
    def export_samples(source_idx, tfr_prefix):
        ...
        if source_idx.shape != (source_idx.shape[0], 2):
            ...
            source_idx = np.stack(
                [np.zeros(source_idx.shape[0], dtype=np.uint8), source_idx], axis=-1
            )
        with TFRecordExporter(
            tfrecord_dir, len(source_idx), tfr_prefix=tfr_prefix
        ) as tfr:
...
def extract_brecahad_crops(brecahad_dir, output_dir, cropsize=256):
    params = {256: {'overlap': 0.0}, 512: {'overlap': 0.5}}
...
def execute_cmdline(argv):
    ...
    p = add_command(
        'info', 'Display general info about dataset.', 'info datasets/mnist'
    )
    ...
    p = add_command(
        'convert_to_hdf5',
        'Convert dataset to legacy HDF5 archive.',
    ...
    p.add_argument(
        '--compress', help='Compress the data (default: 0)', type=int, default=0
    )

    p = add_command(
        'hdf5_from_images',
        'Create HDF5 archive from a directory of images.',
    ...
    p = add_command(
        'unpack',
        'Unpack a TFRecords dataset to labels and images for later repackaging with `pack`.',
    )
    p.add_argument(
        '--tfrecord_dir',
        help='Directory containing the source dataset in TFRecords format',
        required=True,
    )
    p.add_argument(
        '--output_dir',
        help='Output directory where to extract the dataset as PNG files',
        required=True,
    )
    p.add_argument(
        '--resolution',
        help='Resize images to (resolution,resolution) (default: None = no resizing)',
        type=int,
        default=None,
    )
    ...
    p.add_argument(
        '--unpacked_dir',
        help='Source directory containing an unpacked tfrecords dataset',
    )
    p.add_argument('--tfrecord_dir', help='New dataset directory to be created')
    p.add_argument(
        '--num_train',
        help='Number of images to pick for the training set (default: None = all)',
        type=int,
        default=None,
    )
    p.add_argument(
        '--num_validation',
        help='Number of images to pick for the validation set (default: 0 = no images)',
        type=int,
        default=0,
    )
    p.add_argument(
        '--mirror',
        help='Number of images to pick for the training set (default: 0 = no mirroring)',
        type=int,
        default=0,
    )
    p.add_argument(
        '--seed',
        help='Shuffle random seed.  (default: None = do not shuffle)',
        type=int,
        default=None,
    )

    p = add_command(
        'extract_brecahad_crops', 'Extract crops from the original BreCaHAD images'
    )
    p.add_argument(
        '--brecahad_dir',
        help='Source directory for BreCaHAD images.  Should contain .tif files.',
        required=True,
    )
    p.add_argument(
        '--output_dir',
        help='Output directory for image crops.  Will contain .png files',
        required=True,
    )
    p.add_argument(
        '--cropsize', help='Crop size (resolution,resolution)', type=int, default=256
    )
