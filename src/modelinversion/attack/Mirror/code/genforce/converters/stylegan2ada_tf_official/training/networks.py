...
def get_weight(
    ...
    fan_in = np.prod(
        shape[:-1]
    )  # [kernel, kernel, fmaps_in, fmaps_out] for conv2d, [in, out] for fully-connected.
    ...
    w = (
        tf.get_variable(weight_var, shape=shape, initializer=init, trainable=trainable)
        * runtime_coef
    )
...
def conv2d(x, w, up=False, down=False, resample_kernel=None, padding=0):
    ...
    if up:
        x = upsample_conv_2d(
            x, w, data_format='NCHW', k=resample_kernel, padding=padding
        )
    elif down:
        ...
        x = tf.nn.conv2d(
            x, w, data_format='NCHW', strides=[1, 1, 1, 1], padding=padding_mode
        )
    return x
...
def conv2d_layer(
    ...
    return conv2d(
        x, tf.cast(w, x.dtype), up=up, down=down, resample_kernel=resample_kernel
    )
...
def modulated_conv2d_layer(
    ...
    w = get_weight(
        wshape, lrmul=lrmul, trainable=trainable, use_spectral_norm=use_spectral_norm
    )
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
        w *= np.sqrt(1 / np.prod(wshape[:-1])) / tf.reduce_max(
            tf.abs(w), axis=[0, 1, 2]
        )  # Pre-normalize to avoid float16 overflow.
    ...
    s = dense_layer(
        ...
        weight_var='mod_weight',
        trainable=trainable,
        use_spectral_norm=use_spectral_norm,
    )  # [BI] Transform incoming W to style.
    s = (
        apply_bias_act(s, bias_var='mod_bias', trainable=trainable) + 1
    )  # [BI] Add bias (initially 1).
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
    ...
    **_kwargs,  # Ignore unrecognized keyword args.
    ...
    for layer_idx in range(mapping_layers):
        with tf.variable_scope(f'Dense{layer_idx}'):
            fmaps = (
                mapping_fmaps
                if mapping_fmaps is not None and layer_idx < mapping_layers - 1
                else dlatent_size
            )
            x = apply_bias_act(
    ...
    **_kwargs,  # Ignore unrecognized keyword args.
    ...
    if label_size > 0:
        ...
        for idx in range(mapping_layers):
            with tf.variable_scope(f'Mapping{idx}'):
                y = apply_bias_act(
                    dense_layer(y, fmaps=mapping_fmaps, lrmul=mapping_lrmul),
                    act=act,
                    lrmul=mapping_lrmul,
                )
        labels_in = y
    ...
    def adrop(x):
        if adaptive_dropout != 0:
            ...
            x *= tf.cast(
                tf.exp(tf.random_normal(s) * (augment_strength * adaptive_dropout)),
                x.dtype,
            )
        return x
    ...
    with tf.variable_scope('4x4'):
        ...
        with tf.variable_scope('Dense0'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(
                adrop(dense_layer(x, fmaps=nf(0), trainable=trainable)),
                act=act,
                trainable=trainable,
            )
    ...
    with tf.variable_scope('Output'):
        if label_size > 0:
            ...
            x = tf.reduce_sum(x * labels_in, axis=1, keepdims=True) / np.sqrt(
                mapping_fmaps
            )
        else:
