...
def knn_precision_recall_features(
    ...
    state.precision, state.realism_scores, state.nearest_neighbors = (
        state.ref_manifold.evaluate(
...
class PR(metric_base.MetricBase):
    ...
    def _evaluate(
        ...
        with dnnlib.util.open_url(
            'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada/pretrained/metrics/vgg16.pkl'
        ) as f:
        ...
        if os.path.isfile(cache_file):
            ...
            for images, _labels, num in self._iterate_reals(minibatch_size):
                ...
                feat_real += list(
                    feature_net.run(images, num_gpus=num_gpus, assume_frozen=True)
                )[:num]
        ...
        state = knn_precision_recall_features(
            ref_features=feat_real,
            eval_features=feat_fake,
