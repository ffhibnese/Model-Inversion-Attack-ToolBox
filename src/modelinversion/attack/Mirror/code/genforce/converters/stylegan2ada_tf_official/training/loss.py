...
def report_loss(aug, G_loss, D_loss, G_reg=None, D_reg=None):
    ...
    if G_reg is not None:
        terms.G_reg = report_stat(aug, 'Loss/G/reg', G_reg)
    if D_reg is not None:
        terms.D_reg = report_stat(aug, 'Loss/D/reg', D_reg)
    return terms
...
def stylegan2(
    ...
    **_kwargs,
    ...
    with tf.name_scope('Loss_main'):
        G_loss = tf.nn.softplus(
            -D_fake.scores
        )  # -log(sigmoid(D_fake.scores)), pylint: disable=invalid-unary-operand-type
        D_loss = tf.nn.softplus(D_fake.scores)  # -log(1 - sigmoid(D_fake.scores))
        D_loss += tf.nn.softplus(
            -D_real.scores
        )  # -log(sigmoid(D_real.scores)), pylint: disable=invalid-unary-operand-type
        G_reg = 0
        D_reg = 0
    ...
    if pl_weight != 0:
        with tf.name_scope('Loss_PL'):
            ...
            # Compute |J*y|.
            pl_noise = tf.random_normal(tf.shape(G_pl.images)) / np.sqrt(
                np.prod(G.output_shape[2:])
            )
            pl_grads = tf.gradients(
                tf.reduce_sum(G_pl.images * pl_noise), [G_pl.dlatents]
            )[0]
            pl_lengths = tf.sqrt(
            ...
            with tf.control_dependencies(None):
                pl_mean_var = tf.Variable(
                    name='pl_mean', trainable=False, initial_value=0, dtype=tf.float32
                )
            pl_mean = pl_mean_var + pl_decay * (
                tf.reduce_mean(pl_lengths) - pl_mean_var
            )
    ...
    **_kwargs,
    ...
    if r1_gamma != 0 or r2_gamma != 0:
        with tf.name_scope('Loss_R1R2'):
            if r1_gamma != 0:
                ...
                D_reg += r1_penalty * (r1_gamma * 0.5)
            if r2_gamma != 0:
                r2_grads = tf.gradients(tf.reduce_sum(D_fake.scores), [G_fake.images])[
                    0
                ]
    ...
    if (bcr_real_weight != 0 or bcr_fake_weight != 0) and bcr_augment is not None:
        with tf.name_scope('Loss_bCR'):
            if bcr_real_weight != 0:
                bcr_real_images, bcr_real_labels = dnnlib.util.call_func_by_name(
                    D_real.images_aug, D_real.labels_aug, **bcr_augment
                )
                D_bcr_real = eval_D(
                    D,
                    aug,
                ...
                bcr_real_penalty = report_stat(
                    aug, 'Loss/bcr_penalty/real', bcr_real_penalty
                )
                D_loss += (
                    bcr_real_penalty * bcr_real_weight
                )  # NOTE: Must not use lazy regularization for this term.
            if bcr_fake_weight != 0:
                bcr_fake_images, bcr_fake_labels = dnnlib.util.call_func_by_name(
                    D_fake.images_aug, D_fake.labels_aug, **bcr_augment
                )
                D_bcr_fake = eval_D(
                    D,
                    aug,
                ...
                bcr_fake_penalty = report_stat(
                    aug, 'Loss/bcr_penalty/fake', bcr_fake_penalty
                )
                D_loss += (
                    bcr_fake_penalty * bcr_fake_weight
                )  # NOTE: Must not use lazy regularization for this term.
    ...
    if zcr_gen_weight != 0 or zcr_dis_weight != 0:
        with tf.name_scope('Loss_zCR'):
            zcr_fake_latents = (
                fake_latents
                + tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
                * zcr_noise_std
            )
            G_zcr = eval_G(G, zcr_fake_latents, fake_labels)
            if zcr_gen_weight > 0:
                zcr_gen_penalty = -tf.reduce_mean(
                    tf.square(G_fake.images - G_zcr.images), axis=[1, 2, 3]
                )
                zcr_gen_penalty = report_stat(
                    aug, 'Loss/zcr_gen_penalty', zcr_gen_penalty
                )
            ...
            if zcr_dis_weight > 0:
                ...
                zcr_dis_penalty = report_stat(
                    aug, 'Loss/zcr_dis_penalty', zcr_dis_penalty
                )
    ...
    if auxrot_alpha != 0 or auxrot_beta != 0:
        with tf.name_scope('Loss_AuxRot'):
            ...
            if auxrot_alpha != 0:
                ...
                auxrot_fake = tf.where(
                    b2, auxrot_fake, tf.transpose(auxrot_fake, [0, 1, 3, 2])
                )
                D_auxrot_fake = eval_D(D, aug, auxrot_fake, fake_labels, return_aux=4)
                G_loss += (
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        labels=idx, logits=D_auxrot_fake.aux
                    )
                    * auxrot_alpha
                )
            if auxrot_beta != 0:
                ...
                auxrot_real = tf.where(
                    b2, auxrot_real, tf.transpose(auxrot_real, [0, 1, 3, 2])
                )
                D_auxrot_real = eval_D(D, aug, auxrot_real, real_labels, return_aux=4)
                D_loss += (
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        labels=idx, logits=D_auxrot_real.aux
                    )
                    * auxrot_beta
                )
...
def wgangp(
    ...
    **_kwargs,
    ...
    with tf.name_scope('Loss_epsilon'):
        epsilon_penalty = report_stat(
            aug, 'Loss/epsilon_penalty', tf.square(D_real.scores)
        )
        D_loss += epsilon_penalty * wgan_epsilon

    # Gradient penalty from "Improved Training of Wasserstein GANs".
    with tf.name_scope('Loss_GP'):
        mix_factors = tf.random_uniform(
            [minibatch_size, 1, 1, 1], 0, 1, dtype=G_fake.images.dtype
        )
        mix_images = tflib.lerp(
            tf.cast(real_images, G_fake.images.dtype), G_fake.images, mix_factors
        )
        mix_labels = (
            real_labels  # NOTE: Mixing is performed without respect to fake_labels.
        )
