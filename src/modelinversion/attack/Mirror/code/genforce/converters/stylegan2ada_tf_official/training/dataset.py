...
class TFRecordDataset:
    def __init__(
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'), tf.control_dependencies(
            None
        ):
    ...
    def get_minibatch_tf(self):
        ...
        if self.mirror_augment:
            ...
            images = tf.where(
                tf.random_uniform([tf.shape(images)[0]]) < 0.5,
                images,
                tf.reverse(images, [3]),
            )
    ...
    def get_minibatch_np(
        self, minibatch_size, lod=0
    ):  # => (images, labels) or (None, None)
    ...
    def load_validation_set_np(self):
        ...
        if self.has_validation_set:
            validation_set = TFRecordDataset(
                ...
                resolution=self.shape[2],
                max_label_size=self.label_size,
                max_images=self._max_validation,
                repeat=False,
                shuffle=False,
                prefetch_mb=0,
                _is_validation=True,
        ...
        labels = (
            ...
            if len(labels)
            else np.zeros([0, self.label_size], self.label_dtype)
        )
