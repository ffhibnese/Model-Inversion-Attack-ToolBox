...
class Network:
    ...
    def _init_fields(
        ...
        self._var_inits = (
            dict()
        )  # var_name => initial_value, set to None by _init_graph()
        self._all_inits_known = (
            False  # Do we know for sure that _var_inits covers all the variables?
        )
        self._components = (
            None  # subnet_name => Network, None if the components are not known yet
        )
    ...
    def _init_graph(self) -> None:
        ...
        ), tfutil.absolute_name_scope(self.scope), tf.device(
            self.device
        ...
        remaining_inits = [
            var.initializer
            for name, var in self._own_vars.items()
    ...
    def input_names(self):
        ...
        if self._input_names is None:
            self._input_names = [
                t.name.split("/")[-1].split(":")[0] for t in self.input_templates
            ]
    ...
    def _get_vars(self):
        if self._vars is None:
            ...
            for comp in self._get_components().values():
                self._vars.update(
                    (comp.name + "/" + name, var)
                    for name, var in comp._get_vars().items()
                )
        return self._vars
    ...
    def _get_var_global_to_local(self):
        if self._var_global_to_local is None:
            self._var_global_to_local = OrderedDict(
    ...
    def find_var(self, var_or_local_name: Union[TfExpression, str]) -> TfExpression:
        ...
        return (
            self._get_vars()[var_or_local_name]
    ...
    def __getstate__(self) -> dict:
        ...
        state["variables"] = list(
            zip(
                self._get_own_vars().keys(),
                tfutil.run(list(self._get_own_vars().values())),
            )
        )
    ...
    def copy_own_vars_from(self, src_net: "Network") -> None:
        ...
        if (
            src_net._var_inits is not None and not src_net._all_inits_known
        ) or src_net._components is None:
            src_net._get_vars()
        ...
        if src_net._var_inits is None and self._var_inits is None:
            names = [
                name
                for name in self._get_own_vars().keys()
                if name in src_net._get_own_vars()
            ]
            tfutil.set_vars(
                tfutil.run(
                    {
                        self._get_vars()[name]: src_net._get_vars()[name]
                        for name in names
                    }
                )
            )
    ...
    def copy_vars_from(self, src_net: "Network") -> None:
        ...
        if src_net._var_inits is None and self._var_inits is None:
            names = [
                name for name in self._get_vars().keys() if name in src_net._get_vars()
            ]
            tfutil.set_vars(
                tfutil.run(
                    {
    ...
    def copy_trainables_from(self, src_net: "Network") -> None:
        ...
        names = [
            name
            for name in self._get_trainables().keys()
            if name in src_net._get_trainables()
        ]
        tfutil.set_vars(
            tfutil.run(
                {self._get_vars()[name]: src_net._get_vars()[name] for name in names}
            )
        )
    ...
    def list_layers(self) -> List[Tuple[str, TfExpression, List[TfExpression]]]:
        ...
        def recurse(scope, parent_ops, parent_vars, level):
            ...
            if (level == 0 or not contains_direct_ops) and (
                len(cur_ops) != 0 or len(cur_vars) != 0
            ):
                visited = set()
