...
def get_plugin(cuda_file, extra_nvcc_options=[]):
    ...
    try:
        ...
        if os.name == 'nt':
            ...
            compile_opts += (
                f' --compiler-options \'{" ".join(tf.sysconfig.get_compile_flags())}\''
            )
            compile_opts += (
                f' --linker-options \'{" ".join(tf.sysconfig.get_link_flags())}\''
            )
        else:
        ...
        cache_dir = (
            util.make_cache_dir_path('tflib-cudacache')
            if cuda_cache_path is None
            else cuda_cache_path
        )
