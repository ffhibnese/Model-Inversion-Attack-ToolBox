...
class Projector:
    ...
    def set_network(self, Gs, dtype='float16'):
        if Gs is None:
            ...
            return
        self._Gs = Gs.clone(
            randomize_noise=False, dtype=dtype, num_fp16_res=0, fused_modconv=True
        )
        ...
        dlatents_noise = (
            tf.random.normal(shape=self._dlatents_var.shape) * self._dlatent_noise_in
        )
    ...
    def start(self, target_images):
        ...
        tflib.set_vars(
            {self._target_images_var: target_images, self._dlatents_var: dlatents}
        )
    ...
    def step(self):
        ...
        feed_dict = {
            self._dlatent_noise_in: dlatent_noise,
...
def project(
    ...
    if save_video:
        writer = imageio.get_writer(
            f'{outdir}/proj.mp4', mode='I', fps=60, codec='libx264', bitrate='16M'
        )
    ...
    with tqdm.trange(proj.num_steps) as t:
        for step in t:
            ...
            if writer is not None:
                writer.append_data(
                    np.concatenate([target_uint8, proj.images_uint8[0]], axis=1)
                )
            dist, loss = proj.step()
...
def main():
    ...
    parser.add_argument(
        '--save-video',
        help='Save an mp4 video of optimization progress (default: true)',
        type=_str_to_bool,
        default=True,
    )
