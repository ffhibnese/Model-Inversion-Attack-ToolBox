...
class AdaptiveAugment:
    def __init__(
        ...
        assert (
            apply_func is None
            or isinstance(apply_func, str)
            or dnnlib.util.is_top_level_function(apply_func)
        )

        # Configuration.
        self.apply_func = (
            dnnlib.util.get_obj_by_name(apply_func)
    ...
    def init_validation_set(self, D_gpus, training_set):
        ...
        with tflib.absolute_name_scope('Validation'), tf.control_dependencies(None):
            with tf.device('/cpu:0'):
                self._valid_images_in = tf.placeholder(
                    training_set.dtype,
                    name='valid_images_in',
                    shape=[None] + training_set.shape,
                )
                self._valid_labels_in = tf.placeholder(
                    training_set.label_dtype,
                    name='valid_labels_in',
            ...
            ops = []
            for gpu, (D_gpu, images_in_gpu, labels_in_gpu) in enumerate(
                zip(D_gpus, images_in_gpus, labels_in_gpus)
            ):
                with tf.device(f'/gpu:{gpu}'):
                    images_expr = tf.cast(images_in_gpu, tf.float32) * (2 / 255) - 1
                    D_valid = loss.eval_D(
                        D_gpu, self, images_expr, labels_in_gpu, report='valid'
                    )
                    ops += [D_valid.scores]
    ...
    def apply(self, images, labels, enable=True):
        if (
            not enable
            or self.apply_func is None
            or (self.strength == 0 and self.tune_heuristic is None)
        ):
            return images, labels
        with tf.name_scope('Augment'):
            images, labels = self.apply_func(
                images, labels, strength=self.get_strength_var(), **self.apply_args
            )
        return images, labels
    ...
    def get_strength_var(self):
        if self._strength_var is None:
            with tflib.absolute_name_scope('Augment'), tf.control_dependencies(None):
                self._strength_var = tf.Variable(
                    np.float32(self.strength), name='strength', trainable=False
                )
    ...
    def tune(self, nimg_delta):
        acc = {
            name: self._read_and_decay_acc(name, nimg_delta)
            for name in self._tune_stats
        }
    ...
    def run_validation(self, minibatch_size):
        if self._valid_images is not None:
            indices = [
                (self._valid_ofs + i) % self._valid_images.shape[0]
                for i in range(minibatch_size)
            ]
            tflib.run(
                self._valid_op,
                {
                    self._valid_images_in: self._valid_images[indices],
    ...
    def _increment_acc(self, name, expr):
        with tf.name_scope('acc_' + name):
            with tf.control_dependencies(None):
                acc_var = tf.Variable(
                    tf.zeros(2), name=name, trainable=False
                )  # [acc_num, acc_sum]
        ...
        if nimg_delta > 0:
            with tflib.absolute_name_scope('Augment'), tf.control_dependencies(None):
                if self._acc_decay_in is None:
                    self._acc_decay_in = tf.placeholder(
                        tf.float32, name='acc_decay_in', shape=[]
                    )
                if name not in self._acc_decay_ops:
                    with tf.name_scope('acc_' + name):
                        ops = [
                            tf.assign(var, var * self._acc_decay_in) for var in acc_vars
                        ]
                        self._acc_decay_ops[name] = tf.group(*ops)
            acc_decay = (
                0.5 ** (nimg_delta / (self.stat_decay_kimg * 1000))
                if self.stat_decay_kimg > 0
                else 0
            )
...
def gate_augment_params(probability, params, disabled_val):
    ...
    disabled_val = tf.broadcast_to(
        tf.convert_to_tensor(disabled_val, dtype=params.dtype), shape
    )
...
def translate_2d(tx, ty):
    return construct_batch_of_matrices([1, 0, tx], [0, 1, ty], [0, 0, 1])
...
def scale_2d(sx, sy):
    return construct_batch_of_matrices([sx, 0, 0], [0, sy, 0], [0, 0, 1])
...
def rotate_2d(theta):
    return construct_batch_of_matrices(
        [tf.cos(theta), tf.sin(-theta), 0], [tf.sin(theta), tf.cos(theta), 0], [0, 0, 1]
    )
...
def augment_pipeline(
    ...
    if scale > 0:
        ...
        if debug_percentile is not None:
            s = 2 ** (
                tflib.erfinv(tf.broadcast_to(debug_percentile, [batch]) * 2 - 1)
                * scale_std
            )
    ...
    p_rot = 1 - tf.sqrt(
        tf.cast(tf.maximum(1 - rotate * strength, 0), tf.float32)
    )  # P(pre OR post) = p
    if rotate > 0:
        ...
        if debug_percentile is not None:
            theta = (
                (tf.broadcast_to(debug_percentile, [batch]) * 2 - 1)
                * np.pi
                * rotate_max
            )
    ...
    if xfrac > 0:
        ...
        if debug_percentile is not None:
            t = (
                tflib.erfinv(tf.broadcast_to(debug_percentile, [batch, 2]) * 2 - 1)
                * xfrac_std
            )
    ...
    if G_inv is not I_3:
        ...
        cy = (height - 1) / 2
        cp = np.transpose(
            [[-cx, -cy, 1], [cx, -cy, 1], [cx, cy, 1], [-cx, cy, 1]]
        )  # [xyz, idx]
        ...
        images = tf.nn.depthwise_conv2d_backprop_input(
            input_sizes=shape,
        ...
        G_inv = (
            scale_2d(2, 2) @ G_inv @ scale_2d_inv(2, 2)
        )  # Account for the increased resolution.
    ...
    if hue > 0 and channels > 1:
        ...
        if debug_percentile is not None:
            theta = (
                (tf.broadcast_to(debug_percentile, [batch]) * 2 - 1) * np.pi * hue_max
            )
    ...
    if C is not I_4:
        ...
        if channels == 3:
            ...
            images = (
                images * tf.reduce_sum(C[:, :, :3], axis=2, keepdims=True) + C[:, :, 3:]
            )
        else:
    ...
    if imgfilter > 0:
        ...
        for i, band_strength in enumerate(imgfilter_bands):
            ...
            t = tf.concat(
                [t[:, :i], t_i[:, np.newaxis], t[:, i + 1 :]], axis=-1
            )  # Replace i'th element.
            t /= tf.sqrt(
                tf.reduce_sum(expected_power * tf.square(t), axis=-1, keepdims=True)
            )  # Normalize power.
        ...
        for i in range(1, num_bands):
            Hz_bands = np.dstack([Hz_bands, np.zeros_like(Hz_bands)]).reshape(
                num_bands, -1
            )[:, :-1]
            Hz_bands = scipy.signal.convolve(Hz_bands, [Hz_lo2])
            Hz_bands[
                i,
                (Hz_bands.shape[1] - Hz_hi2.size)
                // 2 : (Hz_bands.shape[1] + Hz_hi2.size)
                // 2,
            ] += Hz_hi2
        ...
        Hz_prime = tf.tile(
            Hz_prime[:, :, np.newaxis], [1, 1, channels]
        )  # [tap, batch, channels]
        Hz_prime = tf.reshape(
            Hz_prime, [-1, batch * channels, 1]
        )  # [tap, batch * channels, 1]
