...
def locate_run_dir(run_id_or_run_dir):
    ...
    for search_dir in ['']:
        full_search_dir = (
            config.result_dir
            if search_dir == ''
            else os.path.normpath(os.path.join(config.result_dir, search_dir))
        )
        ...
        run_dirs = [
            ...
            if run_dir_pattern.match(os.path.basename(run_dir))
        ]
...
def load_network_pkl(run_id_or_run_dir_or_network_pkl, snapshot_or_network_pkl=None):
    return load_pkl(
        locate_network_pkl(run_id_or_run_dir_or_network_pkl, snapshot_or_network_pkl)
    )
...
def parse_config_for_previous_run(run_id):
    ...
    if 'file_pattern' in cfg['dataset']:
        cfg['dataset']['tfrecord_dir'] = (
            cfg['dataset'].pop('file_pattern').replace('-r??.tfrecords', '')
        )
