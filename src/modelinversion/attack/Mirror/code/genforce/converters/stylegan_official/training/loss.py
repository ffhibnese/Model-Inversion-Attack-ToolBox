...
def D_hinge(
    ...
    real_scores_out = autosummary('Loss/scores/real', real_scores_out)
    fake_scores_out = autosummary('Loss/scores/fake', fake_scores_out)
    loss = tf.maximum(0.0, 1.0 + fake_scores_out) + tf.maximum(
        0.0, 1.0 - real_scores_out
    )
    return loss
...
def D_logistic_simplegp(
    ...
    if r1_gamma != 0.0:
        with tf.name_scope('R1Penalty'):
            ...
            real_grads = opt.undo_loss_scaling(
                fp32(tf.gradients(real_loss, [reals])[0])
            )
    ...
    if r2_gamma != 0.0:
        with tf.name_scope('R2Penalty'):
            ...
            fake_grads = opt.undo_loss_scaling(
                fp32(tf.gradients(fake_loss, [fake_images_out])[0])
            )
