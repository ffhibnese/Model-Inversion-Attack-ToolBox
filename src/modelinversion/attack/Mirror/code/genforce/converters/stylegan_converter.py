# python3.7
"""Converts StyleGAN model weights from TensorFlow to PyTorch.
...
__all__ = ['convert_stylegan_weight']

GAN_TPYE = 'stylegan'
OFFICIAL_CODE_DIR = 'stylegan_official'
...
def convert_stylegan_weight(
    ...
    for pth_var_name, tf_var_name in G_pth.pth_to_tf_var_mapping.items():
        ...
        if 'weight' in tf_var_name:
            ...
            elif 'Dense' in tf_var_name:
                var = var.permute(1, 0)
            elif 'StyleMod' in tf_var_name:
                var = var.permute(1, 0)
            elif 'LabelConcat' in tf_var_name:
                pass
            elif 'Noise' in tf_var_name:
                pass
            else:
