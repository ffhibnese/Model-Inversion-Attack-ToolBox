...
class LogisticGANLoss(object):
    ...
    def __init__(self, runner, d_loss_kwargs=None, g_loss_kwargs=None):
        ...
        runner.running_stats.add(f'g_loss', log_format='.3f', log_strategy='AVERAGE')
        runner.running_stats.add(f'd_loss', log_format='.3f', log_strategy='AVERAGE')
        if self.r1_gamma != 0:
    ...
    def preprocess_image(images, lod=0, **_unused_kwargs):
        ...
        if int(lod) == 0:
            return images
        return F.interpolate(images, scale_factor=(2 ** int(lod)), mode='nearest')
