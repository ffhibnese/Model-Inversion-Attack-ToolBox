...
class EncoderLoss(object):
    ...
    def __init__(
        self, runner, d_loss_kwargs=None, e_loss_kwargs=None, perceptual_kwargs=None
    ):
        ...
        runner.running_stats.add(f'adv_loss', log_format='.3f', log_strategy='AVERAGE')
        runner.running_stats.add(f'loss_fake', log_format='.3f', log_strategy='AVERAGE')
        runner.running_stats.add(f'loss_real', log_format='.3f', log_strategy='AVERAGE')
        if self.r1_gamma != 0:
            runner.running_stats.add(
    ...
    def compute_grad_penalty(images, scores):
        ...
        image_grad = torch.autograd.grad(
            outputs=scores.sum(), inputs=images, create_graph=True, retain_graph=True
    ...
    def d_loss(self, runner, data):
        ...
        if self.r1_gamma:
            ...
            runner.running_stats.update({'real_grad_penalty': real_grad_penalty.item()})
        if self.r2_gamma:
            fake_grad_penalty = self.compute_grad_penalty(reals_rec, fake_scores)
            runner.running_stats.update({'fake_grad_penalty': fake_grad_penalty.item()})
