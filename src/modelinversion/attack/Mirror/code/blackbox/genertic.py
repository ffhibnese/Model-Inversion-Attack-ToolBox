...
class VectorizedPopulation:
    ...
    def compute_fitness(self):
        with torch.no_grad():
            ...
            assert self.fitness_scores.ndim == 1 and self.fitness_scores.shape[
                0
            ] == len(self.population)
    ...
    def produce_next_generation(self, elite_value):
        with torch.no_grad():
            ...
            self.population = torch.cat(
                [elite_value.unsqueeze(0), mutated_children], dim=0
            )
...
def init_population(
    ...
    target_label,
    target_model,
    compute_fitness_func,
    folder_manager: FolderManager,
):

    w_dir = os.path.join(folder_manager.config.presample_dir, 'w')

    invert_lrelu = nn.LeakyReLU(negative_slope=5.0)
    ...
    select_w, conf = find_closest_latent(
        target_model,
        args.device,
        [target_label],
        k=args.population,
        arch_name=args.target_name,
        presample_dir=folder_manager.config.presample_dir,
        bs=args.presample_batch_size,
    )
    ...
    return VectorizedPopulation(
        ...
        mutation_ce=args.mutation_ce,
        apply_noise_func=apply_noise_func,
        clip_func=clip_func,
        compute_fitness_func=compute_fitness_func,
        bs=args.presample_batch_size,
        device=args.device,
    )
