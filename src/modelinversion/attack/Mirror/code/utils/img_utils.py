...
ALL_STDS = {
    ...
    'vgg16bn': [1.0, 1.0, 1.0],
    # 'resnet50': [1., 1., 1.],
    'resnet50_scratch_dag': [1.0, 1.0, 1.0],
...
def denormalize(image_tensor, arch_name):
    ...
    if arch_name not in [
        ...
        'vgg_face_dag',
    ] and not arch_name.startswith('inception_resnetv1'):

        return image_tensor
    std = ALL_STDS[arch_name]
    mean = ALL_MEANS[arch_name]
    image_tensor = (
        image_tensor * torch.tensor(std, device=image_tensor.device)[:, None, None]
        + torch.tensor(mean, device=image_tensor.device)[:, None, None]
    )
...
def normalize(image_tensor, arch_name):
    ...
    mean = ALL_MEANS[arch_name]
    image_tensor = (
        image_tensor - torch.tensor(mean, device=image_tensor.device)[:, None, None]
    ) / torch.tensor(std, device=image_tensor.device)[:, None, None]
    return image_tensor
...
def clip(image_tensor, use_fp16, arch_name):
    ...
    return (torch.clamp(image_tensor, 0.0, 255.0) - mean) / std
