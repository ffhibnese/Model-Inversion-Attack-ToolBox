...
def load_cls_embed(dataset, cls_path, device, classify=False, logits=False):
    ...
    if dataset == 'mnist':
        ...
        def extract_feat(x):
            assert x.min() >= 0  # in case passing in x in [-1,1] by accident
            return func((x.view(x.size(0), C, H, W) - 0.5) / 0.5)
        ...
        def extract_feat(x, mb=100):
            ...
            for start in range(0, len(x), mb):
                _x = x[start : start + mb]
                zs.append(func((_x.view(_x.size(0), C, H, W) - 0.5) / 0.5))
            return torch.cat(zs)
...
def instantiate_generator(args, device):
    if args.gen in ['basic', 'conditional', 'conditional_no_embed']:
        generator = nets.ConditionalGenerator(
            args.imageSize,
            args.nz,
            args.ngf,
            args.nc,
            args.n_conditions,
            args.gen in ['conditional', 'conditional_no_embed'],
            args.g_sn,
            args.g_z_scale,
            args.g_conditioning_method,
            args.gen != 'conditional_no_embed',
            norm=args.g_norm,
            cdim=args.cdim,
        ).to(device)
    elif args.gen in ['secret', 'secret-conditional']:
        generator = nets.ConditionalGeneratorSecret(
            args.imageSize,
            args.nz,
            args.ngf,
            args.nc,
            args.n_conditions,
            args.gen == 'secret-conditional',
            args.g_sn,
            args.g_z_scale,
            args.g_conditioning_method,
            args.gen != 'conditional_no_embed',
        ...
        generator = nets.ConditionalGeneratorToy(
            args.imageSize,
            args.nz,
            args.ngf,
            args.nc,
...
def instantiate_discriminator(args, index2class, device):
    ...
    discriminator = eval('nets.' + disc_config['name'])(**disc_config['kwargs']).to(
        device
    )
    print(discriminator)
...
def compute_divergences(x1, y1, x2, y2, compute_all=False):
    ...
    def compute_jsd(params_p, params_q, samples_p, samples_q):
        ...
        logq = compute_gmm_loglikelihood(samples_p, params_q[0], params_q[1])
        kl_pm = (
            logp - (torch.logsumexp(torch.stack([logp, logq]), 0) - np.log(2))
        ).mean()
        # KL(Q || .5(P+Q))
        ...
        kl_qm = (
            logq - (torch.logsumexp(torch.stack([logp, logq]), 0) - np.log(2))
        ).mean()
    ...
    return {'kl_12': kl_12, 'kl_21': kl_21, 'jsd': jsd, 'frechet': frechet}
...
class kNNManifold:
    ...
    def compute_fraction_manifold_covered(self, y, k, factor=1):
        ...
        return (
            ((pairwise_dists < r[None].repeat(Ny, 1)).float().sum(0) > 0).float().mean()
        )
...
class PRCD:
    ...
    def evaluate(self, sample_x):
        with torch.no_grad():
            ...
            x1 = torch.cat(
                [
                    self.run_feature_extract(sample_x[start : start + bs])
        ...
        for k in [5, 10]:
            ...
            density = real_manifold.compute_density(x1, k=k, factor=1)
...
class TargetDatasetReconstructionEvaluation:
                ...
                self.target_data_z = torch.cat(
                    [
                        self.evaluation_classifier.embed(
                            ...
                            if self.bgr
                            else self.target_data_x[start : start + bs]
                        ).cpu()
                        for start in range(0, len(self.target_data_x), bs)
                    ]
                )
    ...
    def compute_eval_embeds(self, x):
        return self.evaluation_classifier.embed(
            x[:, [2, 1, 0]] if self.bgr else ((x / 2 + 0.5) - 0.1307) / 0.3081
        )

    def compute_eval_logits(self, x):
        return self.evaluation_classifier.logits(
            x[:, [2, 1, 0]] if self.bgr else ((x / 2 + 0.5) - 0.1307) / 0.3081
        )
    ...
    def evaluate(self, sample_x, sample_y, target_classifier, run_frechet=True):
        ...
        N = len(sample_x)
        if sample_y is None:
            sample_y = target_classifier(sample_x / 2 + 0.5).max(1)[1]
        sample_y = sample_y.cuda()

        x1 = self.evaluation_classifier.embed(
            sample_x[:, [2, 1, 0]]
            if self.bgr
            else ((sample_x / 2 + 0.5) - 0.1307) / 0.3081
        )
        # Frechet Distance
        ...
        top5_acc = np.mean(
            [y in t for y, t in zip(sample_y.cpu().numpy(), top5.cpu().numpy())]
        )
