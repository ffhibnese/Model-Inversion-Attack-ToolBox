...
from model_utils import (
    ...
    load_cls_embed,
    load_cls_z_to_lsm,
    TargetDatasetReconstructionEvaluation,
)
...
def load_gan_from_ckpt(ckpt_path, device):
    with open(os.path.join(os.path.split(ckpt_path)[0], 'args.json'), 'r') as f:
        ...
        if 'nc' not in dir(model_args):
            model_args.nc = 3
        if (
            model_args.model == 'kplus1gan'
            or ('lambda_diversity' in dir(model_args) and model_args.lambda_diversity)
            > 0
        ):
            model_args.cdim = 50
...
class AttackExperiment:
    def __init__(
        self, config_fname, device, db=0, fixed_id=-1, run_target_feat_eval=False, k=5
    ):
        ...
        self.dat = data.load_data(
            self.config['data']['aux'],
            imgsize=self.config['data']['image_size'],
            device=device,
        )
        ...
        if experiment_type == 'fsg':
            if self.config['target_cls']['name'] != 'none':
                with open(
                    os.path.join(
                        os.path.split(self.config['target_cls']['path'])[0], 'args.json'
                    ),
                    'r',
                ) as f:
            ...
            if (
                'target_cls' in self.config
                and self.config['target_cls']['name'] != 'none'
            ):
                if self.config['target_cls']['name'] == 'CelebAPretrained':
                    ...
                    self.cdim = self.target_extract_feat(
                        self.target_dataset["X_train"][:2].cuda() / 2 + 0.5
                    ).shape[-1]

        # Load eval cls
        if 'evaluation_cls' in self.config:
            if (
                self.config['evaluation_cls']['class']
                == 'PretrainedInsightFaceClassifier'
            ):
                ...
                if self.config['evaluation_cls']['class'] != 'none':
                    with open(
                        os.path.join(
                            os.path.split(self.config['evaluation_cls']['path'])[0],
                            'args.json',
                        ),
                        'r',
                    ) as f:
                        ecls_args = json.load(f)
                        ecls_args = SimpleNamespace(**ecls_args)
                    self.evaluation_classifier = get_model(ecls_args, device)[0]
            ...
            self.target_eval_runner = TargetDatasetReconstructionEvaluation(
                self.evaluation_classifier,
                self.target_x,
                self.target_y,
                bgr=bgr,
        ...
        if 'prior_gan' in self.config:
            with open(
                os.path.join(
                    os.path.split(self.config['prior_gan']['g_path'])[0], 'args.json'
                ),
                'r',
            ) as f:
                model_args = json.load(f)
                ...
                if (
                    model_args.model == 'kplus1gan'
                    or (
                        'lambda_diversity' in dir(model_args)
                        and model_args.lambda_diversity
                    )
                    > 0
                ):
                    model_args.cdim = 50
            ...
            generator = instantiate_generator(model_args, device)
            loaded = torch.load(os.path.join(self.config['prior_gan']['g_path']))
            generator.load_state_dict(loaded)
            generator.eval()
