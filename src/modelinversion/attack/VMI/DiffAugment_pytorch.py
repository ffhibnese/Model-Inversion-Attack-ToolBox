...
def rand_translation(x, ratio=(1, 8)):
    shift_x, shift_y = (
        x.size(2) * ratio[0] // ratio[1],
        x.size(3) * ratio[0] // ratio[1],
    )
...
if __name__ == '__main__':
    ...
    # Data
    data_args = utils.load_args(
        os.path.join(os.path.dirname(ic_ckpt_path), 'args.json')
    )
    ...
    dat = data.load_data(
        ...
        data_args.dataroot,
        device=device,
        imgsize=data_args.imageSize,
        Ntrain=data_args.Ntrain,
        Ntest=data_args.Ntest,
    ...
    alled2 = DiffAugment(reals / 2 + 0.5, policy='color,translation,cutout')
