...
def select_highest_pyx_given_x(x, target_logsoftmax, Nperclass, target_id=None):
    with torch.no_grad():
        ...
        for start in range(0, len(x), 1000):
            logp = target_logsoftmax(x[start : start + 1000].cuda() / 2 + 0.5)[
                :, args.fixed_id
            ]
            logps.append(logp)
...
def main(args):
    ...
    for run in range(3):
        ...
        if args.method == 'genmi':
            if args.init_rs:
                ...
                N_init = 10000
                noise0 = args.gan_gd_init_scale * torch.randn(
                    N_init, experiment.gan_args.nz, 1, 1, device=device
                )
                ...
                noise = args.gan_gd_init_scale * torch.randn(
                    args.batchSize, experiment.gan_args.nz, 1, 1, device=device
                )
            ...
            noise_optimizer = optim.SGD(
                [noise],
                lr=args.gan_gd_lr,
                momentum=args.gan_gd_m,
                weight_decay=args.gan_gd_wd,
                nesterov=False,
            )
        ...
        for i in pbar:
            ...
            if i % 100 == 0:
                vutils.save_image(
                    fake[:64],
                    '%s/viz_sample/sample_run%03d_i%03d.jpeg'
                    % (args.output_dir, run, i),
                    normalize=True,
                    nrow=8,
                )
                ...
                pandas.Series(D).to_csv(
                    os.path.join(
                        args.output_dir, f'gan_gd_tdr_metrics_run{run}_i{i}.csv'
                    )
                )
                ...
                plot_csv(
                    ...
                    os.path.join(args.output_dir, f'run{run}', 'iteration_plots.jpeg'),
                )
                ...
                if stats_dict['eval-acc'] > best_eval_acc:
                    ...
                    torch.save(
                        best_noise,
                        os.path.join(args.output_dir, f'best_noise_run{run}.pt'),
                    )
                    torch.save(
                        best_fake,
                        os.path.join(args.output_dir, f'best_fake_run{run}.pt'),
                    )
                    with open(
                        os.path.join(args.output_dir, f'best_iter_run{run}.txt'), 'w'
                    ) as f:
                        f.write(f"{i}")

                torch.save(
                    noise.detach().clone().cpu(),
                    os.path.join(args.output_dir, f'last_noise_run{run}.pt'),
                )
                torch.save(
                    fake.detach().clone().cpu(),
                    os.path.join(args.output_dir, f'last_fake_run{run}.pt'),
                )
    ...
    def _final(fakes, name):
        vutils.save_image(
            fakes[:64],
            f'{args.output_dir}/viz_sample/sample__{name}.jpeg',
            normalize=True,
            nrow=8,
        )
        D = target_eval_runner.evaluate(
    ...
    torch.save(
        best_fakes[best_idx], os.path.join(args.output_dir, "after_rs_100_samples.pt")
    )
    best_idx = select_highest_pyx_given_x(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='genmi',
        choices=['gmi', 'genmi'],
        help='gmi optimizes in x-space, genmi in z-space',
    )
    # Data arguments
    ...
    parser.add_argument(
        '--prior_model', type=str, default='disc', choices=['disc', 'lep', 'tep', '0']
    )
