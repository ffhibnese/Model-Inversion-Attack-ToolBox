...
from .flow_utils import split_feature, compute_same_pad
...
class Permute(nn.Module):
    def __init__(self, num_channels, shuffle):
        ...
        self.register_buffer('permute_indices_inverse', indices_inverse)

    def forward(self, input, reverse=False):
        assert len(input.size()) == 2

        if not reverse:
            input = input[:, self.permute_indices]
            return input
        else:
            return input[:, self.permute_indices_inverse]
...
class Permute2d(nn.Module):
    def __init__(self, num_channels, shuffle):
        super().__init__()
        self.num_channels = num_channels
        indices = torch.arange(self.num_channels - 1, -1, -1, dtype=torch.long)
        indices_inverse = torch.zeros((self.num_channels), dtype=torch.long)
        self.register_buffer('permute_indices', indices)
        self.register_buffer('permute_indices_inverse', indices_inverse)

        for i in range(self.num_channels):
            self.permute_indices_inverse[self.permute_indices[i]] = i

        if shuffle:
            self.reset_indices()

    def reset_indices(self):
        shuffle_idx = torch.randperm(self.permute_indices.shape[0])
        self.permute_indices = self.permute_indices[shuffle_idx]
