...
class FiLMSimpleResidualBlock(SimpleResidualBlock):
    ...
    def forward(self, x, gamma1=None, beta1=None, gamma2=None, beta2=None):
        ...
        if any(is_none):
            if not all(is_none):
                raise ValueError(
                    'Expected either all or none of the FiLM ' 'parameters to be None.'
                )
...
class FiLMResNet(ResNet):
    def __init__(
        ...
        if block != FiLMSimpleResidualBlock:
            raise ValueError(
                'Expected block to be FiLMSimpleResidualBlock ' 'when using FiLMResNet.'
            )
...
def define_backbone_and_adaptors(
    num_blocks_per_layer, num_maps_per_layer, final_feature_map_width, flatten=True
):
    backbone = FiLMResNet(
...
def FiLMResNet10(flatten=True):
    return define_backbone_and_adaptors(
        [1, 1, 1, 1], [64, 128, 256, 512], final_feature_map_width=2, flatten=flatten
    )


def FiLMResNet18(flatten=True):
    return define_backbone_and_adaptors(
        [2, 2, 2, 2], [64, 128, 256, 512], final_feature_map_width=2, flatten=flatten
    )
