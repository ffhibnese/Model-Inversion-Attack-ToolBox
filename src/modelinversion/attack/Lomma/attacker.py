...
class LommaGMIAttacker(BaseAttacker):
    ...
    def prepare_attack(self):
        ...
        for model_name in config.aug_model_names:
            ...
            self.folder_manager.load_state_dict(
                ...
                [
                    'Lomma',
                    f'{config.aug_model_dataset_name}_{config.target_name}_{model_name}.pth',
                ],
                device=config.device,
            )
        ...
        save_path = os.path.join(
            self.folder_manager.config.cache_dir, f'{config.target_name}.pt'
        )
    ...
    def _generate_preg(self, model, save_path, sample_num=5000):
        ...
        data_path = os.path.join(
            self.folder_manager.config.dataset_dir,
