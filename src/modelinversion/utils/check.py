...
def check_shape(tensor: torch.Tensor, expect_shape: Union[list[Optional[int]], list[Optional[int]]], raise_exception=True) -> bool:
    tensor_shape = tensor.shape
    
    if len(tensor_shape) < len(expect_shape):
        if raise_exception:
            raise ShapeException(f'expect ndim >= {len(expect_shape)}, but found {len(tensor_shape)}')
        return False
    # torch.Size().
    ...
    for i in range(len(expect_shape)):
        ...
        if expect_shape[i] != tensor_shape[i]:
            if raise_exception:
                ...
                raise ShapeException(f'expect shape [..., {expect_shape_str}], but found [{tensor_shape_str}]')
