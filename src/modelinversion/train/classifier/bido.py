...
class BiDOTrainer(SimpleTrainer):
    ...
    def __init__(self, config: BiDOTrainConfig, *args, **kwargs) -> None:
        ...
        if config.bido_loss_type == 'hisc':
            ...
            raise RuntimeError(f'loss type `{config.bido_loss_type}` is not supported, valid loss types: `hisc` and `coco`')
    ...
    def calc_loss(self, inputs, result, labels: LongTensor):
        ...
        h_label = self._to_onehot(labels, num_classes).to(self.config.device).view(bs, -1)
        ...
        if HOOK_NAME_HIDDEN not in addition_info:
            raise RuntimeError(f'{HOOK_NAME_HIDDEN} is not contained in the output of the model')
            
        for h_hidden in addition_info[HOOK_NAME_HIDDEN]:
            h_hidden = h_hidden.reshape(bs, -1)
            hidden_input_loss, hidden_output_loss = self.objective_fn(h_hidden, h_label, h_data, 5., self.config.kernel_type)
