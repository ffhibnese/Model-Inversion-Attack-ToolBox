...
parser.add_argument('--target', type=str, default='init', choices=['pii', 'pii-bb', 'gmi', 'ini-wb', 'ini-bb', 'full', 'full-bb', 'abl'], help='optimization objective')
...
parser.add_argument('--save', action='store_true', help='save the filled images and generate virtual dataset')
parser.add_argument('--resume', action='store_true', help='resume a previous run, which will not clear the data already generated')
parser.add_argument('--run_file', type=str, help='only required when resume is set to true')
...
parser.add_argument('--max_iter', type=int, default=1500, help='number of iterations for optimization')
parser.add_argument('--root', type=str, default='./data/img_align_celeba/', help='path of the dataset')
parser.add_argument('--identity_file', type=str, default='./data/identity_CelebA.txt', help='path of the identity file')
parser.add_argument('--latent_dim', type=int, default=100, help='latent space dimension')
parser.add_argument('--img_size', type=int, default=64, help='size of each image dimension')
parser.add_argument('--patch_size', type=int, default=32, help='size of patch for local discriminator')
...
pri_loader = DataLoader(priset, opt.batch_size, drop_last=True, shuffle=True, pin_memory=True, worker_init_fn=seed_worker, generator=g)
...
easy_cutout_area = partial(gen_cutout_area, cutout_size=opt.cutout_size, img_size=opt.img_size, patch_size=opt.patch_size)
...
def init_latent_blackbox(imgs, mask, gt_labels, num):
    ...
    for i in trange(num):
        z = torch.randn(opt.batch_size, 100).to(device)
        z = torch.clamp(z, -1, 1)
        z_dict = torch.cat((z_dict, z.unsqueeze(1)), dim=1) if z_dict is not None else z.unsqueeze(1)
        filled_imgs = Net((inp, z))
        filled_imgs = imgs - imgs * mask + filled_imgs * mask
        ...
        label_dict = torch.cat((label_dict, labels.unsqueeze(-1)), dim=1) if label_dict is not None else labels.unsqueeze(-1)
        conf, _ = torch.max(probs, dim=1, keepdim=True)
        conf_dict = torch.cat((conf_dict, conf), dim=1) if conf_dict is not None else conf
    ...
    for i, iden in enumerate(id_bank):
        ...
        z_bank = torch.cat((z_bank, z_select), dim=0) if z_bank is not None else z_select
...
def init_full(imgs, mask, gt_labels, num, k=5):
    ...
    for i, iden in enumerate(id_bank):
        ...
        if z_potential.shape[0] >= k:
            ...
            z_selects = z_potential[torch.topk(conf_potential, k=z_potential.shape[0]).indices]
        ...
        z_bank = torch.cat((z_bank, z_selects), dim=0) if z_bank is not None else z_selects
...
def attack(imgs: torch.tensor, mask: torch.tensor, gt_labels: torch.tensor):
    ...
    if opt.target == 'pii' or opt.target == 'pii-bb':
        ...
        for i in bar:
            ...
            hole_area = gen_hole_area((opt.patch_size, opt.patch_size), (opt.img_size, opt.img_size))
        ...
        for idx in range(opt.m):
            ...
            for i in bar:
                ...
                bar.set_description('L: {:.3f}, L_dis: {:.3f}, L_id: {:.3f}'.format(loss, loss_dis, loss_id.mean()))
...
def run():
    ...
    for idx, batch in enumerate(pbar):
        ...
        pbar.set_description(f'top1_V: {top1V_avg:.3f}, top5_V: {top5V_avg:.3f}, top1_I: {top1I_avg:.3f}, top5_I: {top5I_avg:.3f},  psnr: {psnr_avg:.3f}')
