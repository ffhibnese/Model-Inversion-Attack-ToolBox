...
from torchvision.transforms import ToTensor, Compose, ColorJitter, RandomResizedCrop, RandomHorizontalFlip, Normalize, CenterCrop, Resize, functional as TF

from modelinversion.models import get_stylegan2data_generator, TorchvisionClassifierModel
from modelinversion.sampler import ImageAugmentSelectLatentsSampler, SimpleLatentsSampler
from modelinversion.utils import unwrapped_parallel_module, augment_images_fn_generator, Logger, MinMaxConstraint
...
if __name__ == '__main__':
    ...
    mapping, generator = get_stylegan2data_generator(stylegan2ada_path, stylegan2ada_ckpt_path, single_w=True)
    
    target_resolution = 224
    eval_resolution = 299
    
    target_model = TorchvisionClassifierModel(target_model_name, num_classes=num_classes)
    eval_model = TorchvisionClassifierModel(eval_model_name, num_classes=num_classes, resolution=299, register_last_feature_hook=True)
    ...
    target_model = nn.parallel.DataParallel(target_model, device_ids=gpu_devices).to(device)
    ...
    all_w_mins = F.leaky_relu(all_p_mins, negative_slope=0.2).detach().requires_grad_(False).to(device)
    all_w_maxs = F.leaky_relu(all_p_maxs, negative_slope=0.2).detach().requires_grad_(False).to(device)
    ...
    accuracy_metric = ImageClassifierAttackAccuracy(
        evaluation_batch_size, eval_model, device=device, description='evaluation', transform=to_eval_transform
    )
    
    
    distance_metric = ImageDistanceMetric(
        evaluation_batch_size, eval_model, eval_dataset, device=device, description='evaluation', save_individual_res_dir=experiment_dir, transform=to_eval_transform
    )
    
    fid_prdc_metric = ImageFidPRDCMetric(
        evaluation_batch_size, eval_dataset, device=device, save_individual_prdc_dir=experiment_dir,
        fid=True, prdc=True, transform=to_eval_transform
    )
