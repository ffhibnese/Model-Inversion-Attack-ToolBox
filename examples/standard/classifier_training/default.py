...
from torchvision.transforms import ToTensor, Compose, ColorJitter, RandomResizedCrop, RandomHorizontalFlip
...
if __name__ == '__main__':
    ...
    target_model = TorchvisionClassifierModel(arch_name=torchvison_model_name, num_classes=num_classes)
    target_model = nn.DataParallel(target_model, device_ids=gpu_devices).to(device)
    
    optimizer = torch.optim.Adam(target_model.parameters(), lr=0.001, betas=[0.9, 0.999])
    ...
    test_dataset = ImageFolder(test_dataset_path, ToTensor())
