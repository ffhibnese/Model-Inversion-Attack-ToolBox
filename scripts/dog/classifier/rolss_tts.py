...
from modelinversion.models import TorchvisionClassifierModel, SkipConnectionWrapper
...
def main(skip_coef, skip_num):
    resolution = 224
    # prepare dataset

    num_classes = 530
    model_name = 'resnet152'
    save_name = f'stanford_dogs_{model_name}_rolss{skip_coef}_{skip_num}.pth'
    dataset_path = '/mnt/data/<usrname>/datasets/stanford_dogs/'
    experiment_dir = f'../result_classifier/train_stanford_dogs_{model_name}_rolss{skip_coef}_{skip_num}'
    ...
    model = SkipConnectionWrapper(
        model, residule_keep_ratio=skip_coef, erase_ratio_or_num=skip_num
    )
