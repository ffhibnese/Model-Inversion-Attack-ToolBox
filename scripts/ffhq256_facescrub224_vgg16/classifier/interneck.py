...
def main(neck_dim, neck_activation='tanh', cuda_devices='0', part_index_or_ratio=0.9):
    ...
    save_tag = f'facescrub64_{model_name}_interneck{neck_dim}_{part_index_or_ratio}'
    ...
    backbone_path = '/mnt/data/<usrname>/mywork/lora_defense/test_lora/ffhq64_facescrub64/result_classifier/pretrain_vgg16/pretrain_vgg16.pth'
    ...
    model = InterNeckWrapper(
        model, keep_rank_or_ratio=neck_dim, part_index_or_ratio=part_index_or_ratio
    )
    print(model)
    ...
    train_dataset = FaceScrub(
        ...
        transform=Compose(
            [
                ToTensor(),
                # RandomResizedCrop(
                #     size=(64, 64), scale=(0.85, 1), ratio=(1, 1), antialias=True
                # ),
    ...
    trainer.train(epoch_num, train_loader, test_loader, limit_acc=0.968)
...
if __name__ == '__main__':
    cuda_devices = '4'
    # main(50, neck_activation='leaky_relu', cuda_devices=cuda_devices)

    # necks = [20, 35, 75, 100]
    # necks = [150, 200, 300, 500]
    necks = [20]
    part_index_or_ratios = [-2, 0.8, 0.6, 0.4, 0.2, -1]

    for neck in necks:
        for part_index_or_ratio in part_index_or_ratios:
            main(
                neck,
                neck_activation='tanh',
                cuda_devices=cuda_devices,
                part_index_or_ratio=part_index_or_ratio,
            )
