...
if __name__ == '__main__':
    ...
    lora_dim = 5
    end_lora_dim = 20
    save_name = f'facescrub224_{model_name}_growlora{lora_dim}_{end_lora_dim}.pth'
    dataset_path = '/data/<usrname>/datasets/facescrub/'
    experiment_dir = f'../result_classifier/train_facescrub224_{model_name}_growlora{lora_dim}_{end_lora_dim}'
    ...
    # print(load_res)
    model = GrowLoraWrapper(model, start_lora_dim=lora_dim, end_lora_dim=end_lora_dim)
    ...
    lr_schedular = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[75, 110, 135], gamma=0.2
    )
