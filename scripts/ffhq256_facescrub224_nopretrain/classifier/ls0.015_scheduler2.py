...
if __name__ == '__main__':
    ...
    ls_val = 0.015
    save_name = f'facescrub224_{model_name}_ls{ls_val}_scheduler.pth'
    dataset_path = '/data/<usrname>/datasets/facescrub/'
    experiment_dir = f'../result_classifier/train_facescrub224_{model_name}_ls{ls_val}_scheduler'
    ...
    trainer.train(epoch_num // 2, train_loader, test_loader, save_best_ckpts=False)

    state_2_len = epoch_num // 4
    for i in range(state_2_len):
        trainer.loss_fn = LabelSmoothingCrossEntropyLoss(-ls_val * (i + 1) / state_2_len, save_best_ckpts=False)
