...
from modelinversion.models import TorchvisionClassifierModel, VibWrapper, BiDOWrapper, GrowLoraWrapper
...
if __name__ == '__main__':
    ...
    lora_dim = 5
    end_lora_dim = 20
    save_name = f'facescrub224_{model_name}_growlora{lora_dim}_{end_lora_dim}start2end0.9step1.2.pth'
    dataset_path = '/data/<usrname>/datasets/facescrub/'
    experiment_dir = f'../result_classifier/train_facescrub224_{model_name}_growlora{lora_dim}_{end_lora_dim}start2end0.9step1.2'
    ...
    # print(load_res)
    model = GrowLoraWrapper(model, start_lora_dim=lora_dim, end_lora_dim=end_lora_dim, start_ratio=2, end_ratio=0.9, lora_step=1.2)
    # model = nn.DataParallel(model, device_ids=gpu_devices).to(device)
    model = model.to(device)

    optimizer = torch.optim.Adam(model.optim_nodes.parameters(), lr=0.001, betas=[0.9, 0.999])
