...
def main(tag, cuda_idx, confidence):
    ...
    experiment_dir = f'./if_resnet152{tag}_{confidence:.2f}'
    ...
    weight = target_model.model.fc.weight

    u, s, vh = torch.svd((weight))
    normalize_s = s / s.sum()
    normalize_s_cumsum = torch.cumsum(normalize_s, dim=0)
    rm_mask = normalize_s_cumsum > confidence
    keep_num = torch.sum((~rm_mask).float()).item()
    s[rm_mask] = 0
    # torch.allclose(u @ torch.diag(s) @ vh.t(), weight, atol=1e-5)

    target_model.model.fc.weight.data = (u @ torch.diag(s) @ vh.t())
    ...
    acc = all_acc / all_num
    print(all_acc / all_num)
    with open(os.path.join(experiment_dir, 'acc.txt'), 'w') as f:
        f.write(f"{all_acc} / {all_num} = {acc:.5f}\nkeep num: {keep_num}\n")
...
if __name__ == '__main__':
    ...
    for tag in tags:
        for confidence in [0.25, 0.3, 0,4, 0.5, 0.6, 0.7, 0.8, 0.9]:
            main(tag, 1, confidence)
