...
from modelinversion.models import TorchvisionClassifierModel, ActivationReplacerWrapper
...
if __name__ == '__main__':
    ...
    activation = 'tanh'
    save_name = f'pretrain_{model_name}_{activation}'
    dataset_path = '/data/<usrname>/datasets/msceleb1m/imgs'
    experiment_dir = f'../result_classifier/pretrain_{model_name}_{activation}'
    ...
    model = ActivationReplacerWrapper(model, activation=activation)
    print(model)

    from modelinversion.utils import traverse_module
    traverse_module(model, visit_model)

    print(f'cnt_relu: {cnt_relu}, cnt_tanh: {cnt_tanh}')
