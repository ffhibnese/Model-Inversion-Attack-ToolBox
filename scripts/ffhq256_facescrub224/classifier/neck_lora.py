...
def main(neck_dim, neck_activation='tanh', lora_dim=5, cuda_devices='0'):
    ...
    save_tag = f'facescrub224_{model_name}_lora{lora_dim}start{lora_start}end{lora_end}_neck{neck_dim}{neck_activation}'
    ...
    model = LoraWrapper(model, lora_dim=lora_dim, end_ratio=lora_end, start_ratio=lora_start)
    ...
    trainer.train(epoch_num, train_loader, test_loader, limit_acc=0.975)

    logger.close()

if __name__ == '__main__':
    cuda_devices = '0'
    main(40, cuda_devices=cuda_devices, lora_dim=10)