...
class RandomMixDatasetWrapper(Dataset):
    ...
    def __len__(self):
        return self.mix_length + len(self.dataset)
    

def main(fix_mix, mix_ratio, cenls = 0, focal_alpha=4):
    num_classes = 530
    ...
    # mix_ratio = 1
    if fix_mix:
        fix_mix_tag = 'ramdomfixmix'
    else:
        fix_mix_tag = 'ramdomfreemix'
    focal_gamma = 3

    # cenls = 0
    save_name = f'facescrub224_{model_name}_tl{tl_coef}_focal{focal_gamma}a{focal_alpha}_{fix_mix_tag}{mix_ratio}_cenls{cenls}.pth'
    dataset_path = '/data/<usrname>/datasets/facescrub/'
    experiment_dir = f'../result_classifier/train_facescrub64_{model_name}_tl{tl_coef}_focal{focal_gamma}a{focal_alpha}_{fix_mix_tag}{mix_ratio}_cenls{cenls}'
    ...
    train_dataset = RandomMixDatasetWrapper(train_dataset, mix_length_ratio=mix_ratio, fix_mix=fix_mix)
    ...
    config = MixTrainConfig(
        ...
        mix_loss_fn=InverseFocalLoss(gamma=focal_gamma, alpha=focal_alpha),
