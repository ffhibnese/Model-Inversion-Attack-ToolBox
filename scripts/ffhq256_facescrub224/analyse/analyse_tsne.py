...
from modelinversion.datasets import FaceScrub224, ClassSubset, CelebA224
...
def main(tag):
    ...
    eval_dataset1 = ClassSubset(eval_dataset1, list(range(30)))

    eval_dataset2 = CelebA224(
        "/mnt/data/<usrname>/datasets/pre_celeba_high/private_train",
        output_transform=Compose(
            [
                ToTensor(),
                Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
    ...
    eval_dataset2 = ClassSubset(eval_dataset2, list(range(50)))

    all_features1, all_labels1 = generate_features(model, eval_dataset1)
    all_features2, all_labels2 = generate_features(model, eval_dataset2)

    all_features = np.concatenate([all_features1, all_features2], axis=0)
    all_labels = np.concatenate([all_labels1, all_labels2], axis=0)
    dataset_label = np.concatenate(
        [np.zeros_like(all_labels1), np.ones_like(all_labels2)], axis=0
    )
