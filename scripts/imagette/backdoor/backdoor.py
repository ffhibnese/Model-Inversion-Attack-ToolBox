...
class BlendDataset(PoisonDatasetWrapper):
    ...
    def __init__(self, root, poison_ratio=0.01, blend_ratio=0.1, transform=None, target_transform=None):
        ...
        self.poison_pattern = torch.from_numpy(np.random.RandomState(0).randn(3, 224, 224)).float()
...
def main(dataset_builder: type[PoisonDatasetWrapper], neck_dim: bool = None):
    ...
    save_dir = f'./results/{dataset_builder.__name__}_{neck_dim}'
    ...
    epoch_num = 100
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[75, 90], gamma=0.1)
