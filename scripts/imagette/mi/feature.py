...
def main(
    dataset_builder: type[PoisonDatasetWrapper], save_path: str, feature_compressed
):

    logger = Logger(save_path, 'test.log')

    if feature_compressed:
        output_save_path = os.path.join(save_path, "feature_compressed_before_tanh.pth")
    else:
        output_save_path = os.path.join(save_path, "feature_uncompressed.pth")
    ...
    model = auto_classifier_from_pretrained(
        os.path.join(save_path, "resnet18.pth"),
    ...
    with torch.no_grad():
        for images, labels in testloader:
            feature = model(images.to("cuda"))[1]['feature']
            all_logits.append(feature.detach().cpu())

    all_logits = torch.cat(all_logits, dim=0)
    torch.save(all_logits, output_save_path)
...
if __name__ == '__main__':
    ...
    for f in tqdm(folder):
        if 'None' in f or 'ipynb' in f:
            continue
        result = main(
            CleanDataset, os.path.join("./results2", f), feature_compressed=True
        )
