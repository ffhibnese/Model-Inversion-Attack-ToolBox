...
from modelinversion.train import BiDOTrainer, BiDOTrainConfig
...
if __name__ == '__main__':
    ...
    num_classes = 530
    model_name = 'ir152'
    bido_coef = '0.005_0.05'
    save_name = f'facescrub64_{model_name}_bido{bido_coef}.pth'
    dataset_path = '/data/<usrname>/intermediate-MIA/intermediate-MIA/data/facescrub'
    experiment_dir = (
        f'../result_classifier/train_facescrub64_{model_name}_bido{bido_coef}'
    )
    ...
    ih, oh = bido_coef.split('_')
    ih, oh = float(ih), float(oh)
    config = BiDOTrainConfig(
        ...
        loss_fn=loss_fn,
        coef_hidden_input=ih,
        coef_hidden_output=oh,
    )
