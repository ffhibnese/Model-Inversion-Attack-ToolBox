...
def main(tag):
    ...
    experiment_dir = f'./attack_result2/p2i_ir152_{tag}'
    p2i_adapter_path = f'./results_adapter2/ffhq64_facescrub64/ir152_{tag}'
    ...
    p2i_adapter = auto_adapter_from_pretrained(
        os.path.join(p2i_adapter_path, 'p2i_adapter.pth')
    )
    ...
    latents_sampler = P2ILatentsMixingSampler(
        ...
        output_bias=torch.load(
            os.path.join(p2i_adapter_path, 'avg_w.pth'), map_location='cpu'
        ),
