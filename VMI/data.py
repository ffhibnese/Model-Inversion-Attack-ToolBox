...
def load_data(name, dataroot='data', device='cuda:0', imgsize=64,
              Ntrain=60000, Ntest=10000, n_mixtures=10, radius=3, std=0.05, dataset_size=-1):
...
def create_data(name, data_path, device, imgsize, Ntrain, Ntest, n_mixtures, radius, std):
    if name == 'ring':
        ...
        sample_points = np.random.normal(
            loc=sample_centers, scale=std).astype('float32')
        ...
        dat = {'X_train': X_training, 'Y_train': Y_training,
               'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 10}
        ...
        if name == 'stackedmnist':
            ...
            dat = {'X_train': X_training, 'Y_train': Y_training,
                   'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 1000}
    ...
    #     dat = {'X_train': X_training, 'Y_train': Y_training, 'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 10}
        ...
        dat = {'X_train': X_training, 'Y_train': Y_training,
               'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': -1}
        ...
        dat = {'X_train': None, 'Y_train': None, 'X_test': X_test,
               'Y_test': Y_test, 'nc': nc, 'nclass': len(set(Y_test.numpy()))}
        ...
        for i, x in enumerate(train_loader):
            im = tensor2pil(x[0][0] * -1 / 2 + .5)
            for j in range(4):
                kernel = np.ones((2, 2), np.uint8)
                imd = cv2.dilate(np.array(im), kernel, iterations=j)
                X_training[i*4 + j, :, :,
                           :] = torch.from_numpy((imd / 255.) * 2 - 1)
            if i % 10000 == 0:
                print('Loading data... {}/{}'.format(i, len(train_loader)))
        ...
        dat = {'X_train': X, 'Y_train': None, 'X_test': None,
               'Y_test': None, 'nc': nc, 'nclass': -1}
        ...
        cifar = torchvision.datasets.CIFAR10(
            root=data_path, download=True, transform=transform, train=True)
        train_loader = DataLoader(
            cifar, batch_size=1, shuffle=True, num_workers=0)
        X_training = torch.zeros(len(train_loader), nc, imgsize, imgsize)
        Y_training = torch.zeros(len(train_loader))
        ...
        cifar = torchvision.datasets.CIFAR10(
            root=data_path, download=True, transform=transform, train=False)
        test_loader = DataLoader(
            cifar, batch_size=1, shuffle=False, num_workers=0)
        X_test = torch.zeros(len(test_loader), nc, imgsize, imgsize)
        Y_test = torch.zeros(len(test_loader))
        for i, x in enumerate(test_loader):
            X_test[i, :, :, :] = x[0]
        ...
        dset_imagenet = torchvision.datasets.ImageFolder('data/cinic-10-imagenet/train',
                                                         transform=transform)
        train_loader = DataLoader(
        ...
        dset_imagenet = torchvision.datasets.ImageFolder('data/cinic-10-imagenet/test',
                                                         transform=transform)
        test_loader = DataLoader(
        ...
        dat = {'X_train': X_training, 'Y_train': Y_training,
               'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 5}
        ...
        dset = ImageFolder(os.path.join(
            os.environ['ROOT1'], 'data/pubfig83'), transform=tr)
        ...
        dat = {'X_train': X_train, 'Y_train': Y_train,
               'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 83}
        ...
        X = np.load(os.path.join(
            chestxray.CHESTXRAY_ROOT, size_folder, 'aux_x.npy')).astype('float32') / 255
        ...
        dat = {'X_train': train_x, 'Y_train': train_y, 'X_test': test_x,
               'Y_test': test_y, 'nc': 1, 'nclass': 8}
        ...
        dset = ImageFolder(os.path.join(
            os.environ['ROOT1'], 'data/IIIT-CFW1.0/cartoonFacesFolder'), transform=tr)
        loader = DataLoader(dset, batch_size=1, shuffle=True, num_workers=0)
        X = []
        Y = []
        for i, (x, y) in enumerate(loader):
        ...
        dat = {'X_train': X_train, 'Y_train': Y_train,
               'X_test': X_test, 'Y_test': Y_test, 'nc': nc, 'nclass': 100}
        ...
        dat = {'X_train': X, 'Y_train': Y, 'X_test': None,
               'Y_test': None, 'nc': nc, 'nclass': 64}
        ...
        Y = torch.cat([Y_training, Y_test])
        Y = Y.type('torch.LongTensor')
        dat = {'X_train': None, 'Y_train': None,
               'X_test': X, 'Y_test': Y, 'nc': nc, 'nclass': 16}
        ...
        Y = Y.type('torch.LongTensor')
        dat = {'X_train': None, 'Y_train': None,
               'X_test': X, 'Y_test': Y, 'nc': nc, 'nclass': 20}
        ...
        dat = {'X_train': xtrain, 'Y_train': ytrain, 'X_test': xtest,
               'Y_test': ytest, 'nc': nc, 'nclass': nclass}
        ...
        meta = {
            'image_labels': meta1['image_labels'] + meta2['image_labels'] + meta3['image_labels'],
            'image_names': meta1['image_names'] + meta2['image_names'] + meta3['image_names']
        }
        ...
        dat = {'X_train': None, 'Y_train': None, 'X_test': xs,
               'Y_test': ys, 'nc': nc, 'nclass': nclass}
...
def stack_mnist(data_dir, num_training_sample, num_test_sample, imageSize):
    ...
    X_training = np.zeros(
        shape=(ids.shape[0], imageSize, imageSize, ids.shape[1]))
...
if __name__ == '__main__':
    ...
    omniglot_test = torchvision.datasets.Omniglot(
        root=data_path, download=True, transform=transform, background=False)
    test_loader = DataLoader(omniglot_test, batch_size=32,
